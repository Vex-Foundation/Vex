/**
 * WHAT A MOUNTED WORKSPACE PUBLISHES ABOUT ITSELF - three registries, outside
 * React, each keyed by project.
 *
 * The module is named for what it holds rather than for the first of the three
 * to arrive: a terminal-id INDEX, a workspace LIFECYCLE handle, and the
 * workspace's keyboard-reachable COMMANDS. All three exist for one reason -
 * something that is NOT the workspace needs an operation only the mounted
 * workspace can perform - and all three are single-slot, identity-checked
 * registrations a controller publishes for as long as it lives.
 *
 * ## The terminal index
 *
...
/**
 * THE WORKSPACE ACTIONS A KEYBOARD SHORTCUT CAN REACH.
 *
 * A THIRD registry in this module, and it is here for the reason the first two
 * are: these are things only a MOUNTED workspace can do, asked for by something
 * that is not it. The keyboard table is mounted once by `StudioCenter`
 * (`useStudioKeybindings.ts`), the actions live in the per-project
 * `StudioWorkspaceController`, and the two are in different components with no
 * prop path between them - the same shape as the close gesture, which is why it
 * reuses this seam instead of inventing a second one.
 *
 * SEPARATE FROM {@link ProjectWorkspaceLifecycle}, not folded into it. That
 * handle answers exactly one question - what happens to this workspace's
 * unsaved layout - and is published by an effect on the close closure. These
 * are ordinary user actions published on their own dependencies, and merging
 * the two would make each publisher responsible for not clobbering the other's
 * half.
 *
 * EVERY MEMBER RETURNS whether it acted, and the return is load-bearing: the
 * hook calls `preventDefault` only for a shortcut that was actually taken, so a
 * `Ctrl+W` over a workspace with no tabs reaches the browser rather than being
 * silently eaten by Studio.
 */
export interface ProjectWorkspaceCommands {
  /** Open a terminal in a new tab, as the strip's `+` does. */
  readonly newTerminal: () => boolean;
  /**
   * Split the ACTIVE tab side by side, as the panel header's first split
   * control does. Refuses when the active tab is not a terminal group: there is
   * no side-by-side of a file.
   */
  readonly splitActiveTerminal: () => boolean;
  /** Close the active tab, killing its ptys, as its own close control does. */
  readonly closeActiveTab: () => boolean;
  /** Select the tab `offset` places along, wrapping. */
  readonly selectTabAtOffset: (offset: number) => boolean;
  /**
   * KEEP the active tab, promoting a preview file tab to a pinned one - the
   * keyboard route to the same promotion the tab's double click performs.
   *
   * Answers `false` when there is nothing to keep: an empty strip, a terminal
   * group, or a file tab that is already pinned. That is the honesty rule this
   * whole interface follows and it matters more here than elsewhere: `Enter`
   * is a key the workspace must not swallow, so a `Ctrl+Enter` over an
   * already-pinned tab has to leave the keystroke alone rather than report a
   * promotion that changed nothing.
   */
  readonly pinActiveTab: () => boolean;
}

/** Each mounted workspace's keyboard-reachable actions, keyed by project. */
const commandsByProject = new Map<string, ProjectWorkspaceCommands>();

/**
 * Register this project's keyboard-reachable actions. Returns the unregister.
 *
 * Identity-checked on the way out, like every other single-slot registration
 * here: a controller that unmounted after its successor mounted must not delete
 * the successor's handle.
 */
export function publishProjectWorkspaceCommands(
  projectId: string,
  commands: ProjectWorkspaceCommands,
): () => void {
  commandsByProject.set(projectId, commands);
  return () => {
    if (commandsByProject.get(projectId) === commands) {
      commandsByProject.delete(projectId);
    }
  };
}

/**
 * This project's keyboard-reachable actions, or `null` when no workspace is
 * mounted for it - which is an ordinary state (the welcome screen), not a
 * failure, and is why the caller must be able to decline the keystroke.
 */
export function peekProjectWorkspaceCommands(
  projectId: string,
): ProjectWorkspaceCommands | null {
  return commandsByProject.get(projectId) ?? null;
}
