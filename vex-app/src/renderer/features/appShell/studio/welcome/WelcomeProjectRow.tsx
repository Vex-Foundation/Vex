 ...
 * selected, and its standing permission" for a 256px rail: an icon, a name, a
 * permission label, a drift glyph, and a hover-revealed action cluster. The hero
...
export function WelcomeProjectRow({
  ...
  const description = [
    projectPermissionDescription(project.permission),
    drifted ? projectDriftLabel(project.name, driftSentence) : null,
  ].filter(Boolean).join(" ");
