 ...
 * chart a sliver, so the book column stacks under the ticket instead, with the
 * book and the trades as tabs.
 */
export const LIGHTER_STACK_BELOW = 1100;
...
export const LIGHTER_TICKET_MIN = 260;
...
export function resolveLighterLayout(layout: LighterLayout, frame: LighterDeskFrame): LighterLayoutPixels {
  ...
  const ticketMax = Math.max(LIGHTER_TICKET_MIN, Math.min(shareMax, columnsRoom - bookWidth));
  ...
  const bottomMax = Math.max(
    LIGHTER_BOTTOM_MIN,
    Math.round(Math.min(height * LIGHTER_BOTTOM_MAX_SHARE, height - 1 - LIGHTER_MAIN_MIN_HEIGHT)),
  );
  const bottomHeight = height > 0
    ? clampNumber(height * layout.bottomShare, LIGHTER_BOTTOM_MIN, bottomMax, LIGHTER_BOTTOM_MIN)
    : LIGHTER_BOTTOM_MIN;
  const dockSqueezed = height > 0 && height - 1 - LIGHTER_BOTTOM_MIN < LIGHTER_MAIN_MIN_HEIGHT;
  // The book column above the dock (or its folded bar), less the book/trades seam.
  const column = height - 1 - (layout.bottomCollapsed || dockSqueezed ? LIGHTER_BOTTOM_COLLAPSED : bottomHeight);
  ...
  const tradesHeight = height > 0
    ...
    : LIGHTER_TRADES_MIN;
  return { bookWidth, bookMax, ticketWidth, ticketMax, tradesHeight, tradesMax, bottomHeight, bottomMax, dockSqueezed };
}
