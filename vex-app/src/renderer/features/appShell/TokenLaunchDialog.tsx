    ...
    async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
      ...
      if (outcome.ok) {
        setPhase({
          kind: "done",
          message:
            outcome.data.txHash === null
              ? "Your launch was authorized and is being sent."
              : `Your launch was sent. Transaction ${outcome.data.txHash}.`,
        });
        return;
      }
    ...
    <Dialog open={open} onOpenChange={(next) => (next ? onOpenChange(true) : requestClose())}>
      ...
      <DialogContent className="max-w-[640px]">
        <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
          ...
          <DialogBody>
            <LaunchForm values={values} onChange={setValues} disabled={busy} />
...
function PhaseNote({
  ...
  if (phase.kind === "done") {
    return (
      <p className="text-sm break-all text-[var(--color-success)]" role="status">
        {phase.message}
      </p>
    );
  }
