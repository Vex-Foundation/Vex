 ...
 *
 * `buildPayload` emits jupiter/tavily/rettiwt/relay/indexify exclusively —
 * the IPC contract (`apiKeysSetInputSchema`) has no other fields.
 */
...
export interface FieldRefs {
  ...
  readonly indexify: RefObject<HTMLInputElement | null>;
}
...
export function buildPayload(refs: FieldRefs): ApiKeysSetInput {
  ...
  const indexify = refs.indexify.current?.value.trim() ?? "";
  ...
  return {
    ...
    ...(indexify.length > 0 ? { indexifyApiKey: indexify } : {}),
  };
}
