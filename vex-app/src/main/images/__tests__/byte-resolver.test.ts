...
vi.mock("../locker.js", () => ({ getLockerImage: (id: string) => getLockerImage(id) }));
...
const { createHash } = await import("node:crypto");
const { resolveLockerImageBytesForLaunch, mountLaunchImageByteResolver } = await import(
  "../byte-resolver.js"
);
const seam = await import(
  "@vex-agent/tools/protocols/trench/launch-image-byte-resolver.js"
);
