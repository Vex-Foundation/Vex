/**
 * token-history-db tests — entry-mapping shape coverage (`wallet_intents`
 * inclusion/exclusion + the legacy `proj_activity` entry mapper), split out
 * of `token-history-db.test.ts` by domain under test (Card C5, move-only,
 ...
 * `token-history-db.test.ts`'s own mock setup (mocked `pg`/`db-config`/
 * `@vex-lib/wallet.js`/logger) — deliberately NOT sharing boilerplate across
 * files.
 */
