...
describe("moves-db getMovesForSession — agent_activity half (Agent Scan §4.7)", () => {
  it("unions the agent_activity table, scoped to session_id + wallet + swap/bridge logical rows, with a dedupe guard", async () => {
    ...
    // Swap rows + bridge LOGICAL rows only (Agent Scan Phase 2); allowance/
    // deposit/observed-fill/refund siblings ride `legs`, never a ledger row.
...
describe("moves-db getMovesForSession — agent_activity bridge (Agent Scan Phase 2)", () => {
  it("surfaces the bridge LOGICAL row (event_role IN swap/bridge_fill_expected), product_type from kind, route + legs", async () => {
    ...
    await getMovesForSession(SESSION);
    const sql = String(mocks.query.mock.calls[0]?.[0] ?? "");
    // Only the swap row + the bridge logical row are ledger rows (siblings ride legs).
    expect(sql).toContain("aa.event_role IN ('swap', 'bridge_fill_expected')");
