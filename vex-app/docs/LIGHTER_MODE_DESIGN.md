...
44px 상단 바가 **`← Agent` 뒤로 버튼 + "Lighter" + Core/RHC 전환**을 항상 표시하고, 영구 왼쪽 레일은 예약하지 않는다. `Markets & sessions`는 상단 바 아래의 오버레이 드로어를 열며 차트 폭을 바꾸지 않는다. 내용:
1. **현재 환경** Core / RHC — 상단 세그먼트에서 즉시 전환하며 바와 데스크 팔레트에 텍스트와 색으로 함께 표시.
