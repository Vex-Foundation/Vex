...
/**
 * Explicit operator/user override, or null when only the public fallback
 * exists. `resolveRpcEndpoints` already puts this same override first when
 * one is configured (`getUserRpcOverridesForChain`), so `getLocalChainRpcUrl`
 * does not call this directly — it exists for callers that need to know
 * WHETHER the user configured a private endpoint (e.g. RHC deposit routing),
 * not just which url wins.
 */
