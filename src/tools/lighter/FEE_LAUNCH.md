...
| E5 | Robinhood Chain | Fee authorization approved and submitted on the owner's funded account | Account index, approval id, the four caps and the expiry read back from provider `approved_integrators` | pending |
| E6 | Robinhood Chain | One small approved order, opening a fee-bearing position | Order id, market, filled base and quote value, the integrator fee terms carried on the signed order | pending |
| E7 | Robinhood Chain | One approved cancel of a live order | Order id, provider cancel result, and that no fee was attributed to an unfilled quantity | pending |
