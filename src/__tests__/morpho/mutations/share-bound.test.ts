 ...
 * properties the ruling actually bought: the verdict now uses the ABSOLUTE
 * per-operation bound the approved slippage allows (the same one the on-chain
 * `maxSharePrice` guard enforces), and the raw quoted-vs-settled difference
 * survives as DATA labelled accrual drift rather than as a verdict.
 */
...
describe("morphoShareBoundRaw", () => {
  ...
  it("is a fixed share COUNT for the operation, not a percentage applied to whatever settles", () => {
    // The point of rule 90's ban is a tolerance that stretches to cover a bigger
    // loss. This bound cannot: it is computed ONCE from the quote and the
    // approved bps, so the shortfall it permits is exactly the bps the user
    // agreed to and nothing about the settlement can widen it. Integer division
    // truncates, so the two sizes agree to within one raw unit rather than
    // exactly, and the bound is always rounded in the user's favour.
