...
describe("resolveBorrowOperateRequest", () => {
  it("resolves a deposit-only request (create + deposit): colAmount > 0, debtAmount = 0", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "30000000" });
  ...
  it("resolves a borrow-only request: colAmount = 0, debtAmount > 0", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, positionId: 5, borrowAmountRaw: "5000000" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("resolves a withdraw-only request: colAmount < 0 (negated)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, positionId: 5, withdrawAmountRaw: "1000" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("resolves a repay-only request: debtAmount < 0 (negated)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, positionId: 5, repayAmountRaw: "2000" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("resolves deposit + borrow in one call (the documented two-leg combo)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "30000000", borrowAmountRaw: "5000000" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("defaults market to main and positionId to 0 when omitted", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "1" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("honors an explicit ethena market", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, market: "ethena", depositAmountRaw: "1" });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error("unreachable");
  ...
  it("rejects a missing vaultId", () => {
    const result = resolveBorrowOperateRequest({ depositAmountRaw: "1" });
    expect(result.ok).toBe(false);
  });

  it("rejects an unknown market (reject-not-clamp)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, market: "unknown", depositAmountRaw: "1" });
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("unreachable");
    expect(result.result.output).toMatch(/Unknown market/);
  });

  it("rejects providing more than one of depositAmountRaw/withdrawAmountRaw/withdrawAll", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "1", withdrawAmountRaw: "1" });
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("unreachable");
    expect(result.result.output).toMatch(/at most one of depositAmountRaw, withdrawAmountRaw, withdrawAll/);
  });

  it("rejects providing more than one of borrowAmountRaw/repayAmountRaw/repayAll", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, borrowAmountRaw: "1", repayAll: true });
    expect(result.ok).toBe(false);
  });

  it("rejects a non-positive-integer amount string", () => {
    expect(resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "0" }).ok).toBe(false);
    expect(resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "-5" }).ok).toBe(false);
    expect(resolveBorrowOperateRequest({ vaultId: 1, depositAmountRaw: "abc" }).ok).toBe(false);
  });
  ...
  it("REJECTS combining a collateral deposit with a debt repay (both 'in' — the ledger cannot record two incoming legs)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, positionId: 5, depositAmountRaw: "1", repayAmountRaw: "1" });
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("unreachable");
  ...
  it("REJECTS combining a collateral withdraw with a debt borrow (both 'out')", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, positionId: 5, withdrawAmountRaw: "1", borrowAmountRaw: "1" });
    expect(result.ok).toBe(false);
  });
});
...
describe("buildBorrowOperateIntentParams", () => {
  ...
  it("an untouched leg contributes NO effect entry (never a zero-amount placeholder)", () => {
    const result = resolveBorrowOperateRequest({ vaultId: 1, borrowAmountRaw: "5000000" });
    if (!result.ok) throw new Error("unreachable");
