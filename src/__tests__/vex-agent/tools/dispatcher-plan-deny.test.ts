 ...
 * allowlist (PlanWrite / MissionStop / CompactApply / MissionDraftUpdate)
...
describe("checkPlanAcceptanceDeny — fast path (plan-mode off)", () => {
  it("returns null WITHOUT a DB read when context.planMode is false", async () => {
    ...
    expect(await checkPlanAcceptanceDeny(call("WalletSendConfirm"), offCtx)).toBeNull();
...
describe("checkPlanAcceptanceDeny — gate inactive", () => {
  it("returns null when there is no plan row", async () => {
    ...
    expect(await checkPlanAcceptanceDeny(call("WalletSendConfirm"), ctx)).toBeNull();
  });
...
describe("checkPlanAcceptanceDeny — gate active (enabled, unaccepted)", () => {
  ...
  it("ALLOWS the safe-control allowlist", async () => {
    expect(await checkPlanAcceptanceDeny(call("PlanWrite"), ctx)).toBeNull();
    expect(await checkPlanAcceptanceDeny(call("MissionStop"), ctx)).toBeNull();
    expect(await checkPlanAcceptanceDeny(call("CompactApply"), ctx)).toBeNull();
  });

  // Stage 3 deadlock fix (B3): during mission setup with plan-mode ON, once an
  // enabled+unaccepted plan exists the gate is active. `MissionDraftUpdate`
  ...
  it("ALLOWS MissionDraftUpdate (co-authoring the contract during setup) but still BLOCKS a mutating protocol tool", async () => {
    expect(await checkPlanAcceptanceDeny(call("MissionDraftUpdate"), ctx)).toBeNull();
    expect(await checkPlanAcceptanceDeny(call("PlanWrite"), ctx)).toBeNull();
  ...
  it("ALLOWS read-kind tools (research/discovery)", async () => {
    expect(await checkPlanAcceptanceDeny(call("ToolSearch"), ctx)).toBeNull();
    expect(await checkPlanAcceptanceDeny(call("WebResearch"), ctx)).toBeNull();
  });

  it("BLOCKS a wallet broadcast", async () => {
    const denied = await checkPlanAcceptanceDeny(call("WalletSendConfirm"), ctx);
    expect(denied).not.toBeNull();
    expect(denied!.success).toBe(false);
    expect(denied!.output).toContain("not yet accepted");
  });

  it("BLOCKS a sensitive local write (WalletTrackToken)", async () => {
    // Agent Scan plan v3 (2026-07-23, FIX2-W7): `polymarket_setup` is deleted;
    // `WalletTrackToken` is the surviving registered `actionKind:"local_write"`
    ...
    const denied = await checkPlanAcceptanceDeny(call("WalletTrackToken"), ctx);
    expect(denied).not.toBeNull();
    expect(denied!.success).toBe(false);
  });
