...
describe("kyberswap.swap.quote - a venue-availability failure unlocks the fallback venue", () => {
  beforeEach(resetScaffold);

  it("the EXACT live shape - a geo-block 403 from the edge - reveals the pair and says so", async () => {
  ...
  it("a transport failure that never reached the venue reveals, and says to retry Kyber once first", async () => {
  ...
  it("a 429 reveals, and is also worth one backed-off retry on KyberSwap first", async () => {
  ...
  it("a malformed-params failure does NOT reveal - that is our parameter, not the venue", async () => {
  ...
  it("a price-floor violation does NOT reveal - a fresh quote can clear it", async () => {
  ...
  it("a KYBER_API_ERROR carrying NO http status does NOT reveal", async () => {
  ...
  it("fails closed without a session - nothing revealed, base message intact", async () => {
    mockGetRoute.mockRejectedValueOnce(mapAggregatorError(403, null, "HTTP 403: (html)"));

    const result = await quote({ sessionId: undefined });

    expect(isUniswapPairRevealed(undefined)).toBe(false);
    expect(result.output).not.toContain(FALLBACK_SENTENCE);
    expect(result.output).toContain("refused the request (HTTP 403)");
  });
});

describe("kyberswap.swap.execute - a venue-availability failure reveals AND is recorded", () => {
  beforeEach(resetScaffold);
