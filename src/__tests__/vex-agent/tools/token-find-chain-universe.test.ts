/**
 * `token_find` chain universe — pinned on BOTH description surfaces.
 *
 * The agent reads `token_find` from two places that drifted apart: the protocol
 ...
 * turns asking `token_find` for a Robinhood Chain token, which it cannot know
...
const SURFACES: readonly (readonly [string, string])[] = [
  ["token_find alias", getToolDef("token_find")?.description ?? ""],
  [
...
describe("token_find chain universe", () => {
  ...
  it.each(SURFACES)("%s points at the live list instead of naming a static set", (_name, description) => {
    expect(description).toContain("khalani.chains.list");
  ...
  it.each(SURFACES)("%s names the app-local escape hatches", (_name, description) => {
    ...
    expect(description).toContain("dexscreener.search");
    expect(description).toContain("wallet_track_token");
    expect(description).toContain("wallet_balances");
  });
});
