...
describe("prepared-action follow-up registry", () => {
  ...
  it("rejects Lighter deposit previews whose fund-moving fields were altered", () => {
    const overrides: Record<string, unknown>[] = [
      ...
      { preflightDepositGasLimit: "0" },
      { preflightMaxPriorityFeePerGasWei: "20000000001" },
      { preflightTotalMaxFeeWei: "5999999999999999" },
      { preflightRequiredNativeBalanceWei: "9999999999999999" },
    ];
    for (const override of overrides) {
      expect(
        validatePreparedActionFollowUp("execute_tool", {
