...
describe("buildLighterDepositCalldata", () => {
  ...
  it("encodes an RHC USDG perps deposit to the exact RHC gateway", () => {
    const deployment = getLighterFundingDeployment("rhc");
    const cd = buildLighterDepositCalldata({
      environment: "rhc",
      to: WALLET,
      amountUnits: 1_000_000n,
    });

    expect(cd.to).toBe(deployment.gatewayProxy);
    expect(cd.value).toBe(0n);
    expect(cd.assetIndex).toBe(deployment.settlementAssetIndex);
    expect(cd.routeType).toBe(deployment.perpsRouteType);
    expect(cd.data.startsWith(deployment.depositSelector)).toBe(true);

    const decoded = decodeFunctionData({ abi: LIGHTER_DEPOSIT_FUNCTION_ABI, data: cd.data });
    expect(decoded.args).toEqual([
      getAddress(WALLET),
      deployment.settlementAssetIndex,
      deployment.perpsRouteType,
      1_000_000n,
    ]);
  });
