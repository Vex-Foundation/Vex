...
describe("KYBER_KNOWN_SPENDERS", () => {
  it("contains exactly 1 entry (aggregator router only — limit-order/zap deleted)", () => {
    expect(KYBER_KNOWN_SPENDERS.size).toBe(1);
  });

  it("contains the aggregator router (lowercased)", () => {
...
describe("base URLs", () => {
  const urls = [AGGREGATOR_BASE_URL, TOKEN_API_BASE_URL, COMMON_SERVICE_BASE_URL];
...
describe("timeouts", () => {
  const timeouts = [AGGREGATOR_TIMEOUT_MS, TOKEN_API_TIMEOUT_MS, COMMON_SERVICE_TIMEOUT_MS];
