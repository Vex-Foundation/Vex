...
| `holder-rewards-prepare-distribute.json` | `POST /pools-fun/holder-rewards/prepare` | the CROSS-CHECK's positive case: `to` = the distributor the deployer's event named, `data` = the bare `distribute()` selector `0xe4fc6b6d`, `value` = the string `"0x0"` - byte for byte what Vex builds from the verified distributor ABI. Sanitized: the `wallet` echo carried the Vex probe wallet |
| `holder-rewards-prepare-nothing-to-claim-400.json` | `POST /pools-fun/holder-rewards/prepare` | the provider DECLINING rather than disagreeing: a claim for a wallet owed nothing is HTTP 400 `{"error":"Nothing to claim"}`, a bare `error` with no `details[]`. It maps to `POOLS_INVALID_REQUEST` and is read as `declined`, which never refuses a claim on its own. The response carries no address and is verbatim |
| `chain-holder-rewards-distributor-runtimes.json` | RPC, the distributor runtimes | the MACHINE ARTIFACT behind every wire name in `holder-rewards/{mutations,decode}.ts`: both `claim()` return shapes, both `distribute()` shapes, both `RewardClaimed` topics, `CallerBounty`, every named-error selector, `CALLER_BOUNTY_BPS`, the `locker()` suite binding, and the measured caller-bounty payment that contradicts the launchpad's `paysCallerBounty`. Every topic was read from live logs and confirmed by keccak PREIMAGE; the one unmatched topic is recorded as NOT FOUND rather than guessed |
