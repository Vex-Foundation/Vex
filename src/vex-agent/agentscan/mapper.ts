 ...
 * a stale snapshot can never regress anything.
 */
...
/**
 * The server's closed failure-code enum (contract v1). Vex's own vocabulary is
 * a superset: `solana_signature_expired` (proven blockhash expiry — Solana's
 * flavor of "did not confirm inside its validity window") maps to
 * `confirmation_timeout`; anything else unknown maps to `unknown`.
 */
...
/** Contract v1 event (§4.2) — chain ids as decimal strings (the server coerces bigint). */
...
export function mapActivityToEvent(
  ...
  return {
    ...
    eventRole: str(activity.event_role) ?? "",
    ...
    tokenIn: tokenRef(activity.token_in_address, activity.token_in_symbol, activity.token_in_decimals),
    tokenOut: tokenRef(activity.token_out_address, activity.token_out_symbol, activity.token_out_decimals),
    amountInRaw: guarded(activity.amount_in_raw, RAW_AMOUNT),
    amountOutRaw: guarded(activity.amount_out_raw, RAW_AMOUNT),
    executedInRaw: confirmed ? guarded(activity.executed_amount_in_raw, RAW_AMOUNT) : null,
    executedOutRaw: confirmed ? guarded(activity.executed_amount_out_raw, RAW_AMOUNT) : null,
    ...
    confirmedAt: confirmed ? iso(activity.confirmed_at) : null,
