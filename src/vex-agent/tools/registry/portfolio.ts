...
export const PORTFOLIO_TOOLS: readonly ToolDef[] = [
  {
    ...
    description: [
      ...
      "- `transactions` (PRIMARY view — start here): one chronological feed of your recorded swaps/bridges/orders — pending, confirmed, AND failed — each with amounts (symbols, not raw units), status, chain, and tx hash for an explorer lookup. Filter by `productType` (spot/perps/prediction/bridge/order), keyset-paginate with `cursor` (pass the prior response's `nextCursor` until `hasMore` is false), or pass `txHash` to look up one transaction. Use this for \"what happened recently\" / \"did my last swap land\" / \"find this txHash\".",
      ...
      "Filters narrow the rows: `namespace` (protocol), `productType` (spot/perps/prediction/bridge/order), `txHash`, `cursor`, `limit`.",
    ...
    parameters: { type: "object", properties: {
      ...
      productType: { type: "string", description: "Product filter (e.g. spot, perps, prediction, bridge, order)" },
