/**
 * Venue-fallback eligibility — the COORDINATOR-FIXED classifier deciding which
 * KyberSwap quote/execute failures are ones a SECOND VENUE could actually
 * remedy, and which are conditions a fresh quote or a corrected amount clears.
 * Builders have no discretion to widen or narrow this set.
 *
 * It no longer gates anything (owner decision D4 made `SwapQuoteUniswap` /
 * `SwapExecuteUniswap` always callable). It decides only whether the agent is
 * TOLD about the alternative venue, which matters just as much in the negative
 * direction: naming Uniswap on a slippage or balance failure would be false
 * advice that costs the user a wasted quote.
 *
 * Eligible:
 ...
 *     `deriveKyberMinedRevertFallbackSignal`), not here. A failed receipt
 ...
 *     receipt proves the token/funds cannot be responsible." It only makes
 *     `SwapQuoteUniswap` (a read-only quote probe) worth suggesting; no
 *     automatic fallback execution ever follows (R3).
 ...
 *     `deriveKyberPreSignRevertFallbackSignal`, not here: the leg role must be
...
export type KyberFallbackSignal =
...
const FALLBACK_ELIGIBLE_PRE_SIGN_FAILURE_CODES: ReadonlySet<EvmRouterRevertFailureCode> = new Set([
  "simulation_reverted",
  "route_not_found",
 ...
 */
const FALLBACK_ELIGIBLE_UNAVAILABLE_REASONS: ReadonlySet<KyberVenueUnavailableReason> = new Set([
  "edge_refused", "endpoint_missing", "rate_limited", "server_error", "timeout", "unreachable",
]);

export function isVenueFallbackWorthwhile(failure: KyberFallbackSignal): boolean {
  ...
  if (failure.kind === "pre_sign_revert") {
    return FALLBACK_ELIGIBLE_PRE_SIGN_FAILURE_CODES.has(failure.failureCode);
  }
  if (failure.kind === "venue_unavailable") {
    return FALLBACK_ELIGIBLE_UNAVAILABLE_REASONS.has(failure.reason);
  }
