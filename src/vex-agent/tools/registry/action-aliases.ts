 ...
 * alongside the routers rather than unlocked by a session reveal. The STANDING
 * between the two venues is owned by `swap-venue-guidance.ts` and imported
 * here, never re-worded: they are peers, and the model is told which one
 * usually prices better and why rather than which one is allowed. Nothing is
 * enforced by hiding - the approval gate is what protects the money, and a
 * venue the model cannot see is a venue it cannot reach when the other one
 * refuses the pair. Public schema for all four swap tools:
