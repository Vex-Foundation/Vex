...
export const ACTION_ALIAS_TOOLS: readonly ToolDef[] = [
  {
    ...
    actionKind: "read",
    description:
      `Preview a token swap WITHOUT executing - best route, expected output, price impact, and token-safety signals. EVM chains route to KyberSwap ONLY; chain "solana" routes to Jupiter. EVM tokens must be a CONTRACT ADDRESS or native ETH/native; EVM symbol resolution is disabled here to avoid wrong-contract matches. ${TOKEN_FIND_EVM_IDENTITY_RULE} Solana accepts a symbol or mint. \`amountIn\` is the HUMAN decimal of tokenIn (e.g. "1.5", not wei/lamports). Call this BEFORE SwapExecute: a fresh matching quote (same venue) is what unlocks execution. A ROUTE BEING AVAILABLE DOES NOT MEAN THE WALLET CAN PAY FOR IT: every quote also reads the selected wallet and returns an \`eligibility\` block, and only \`eligibility.executable\` true authorizes an execute. The three balance outcomes are DISTINCT facts with distinct remedies. \`insufficient_balance\` means the wallet holds less of the input token than the trade needs, and names required, current and missing. \`gas_reserve_insufficient\` means the input side is covered but the wallet cannot cover the swap's TOTAL NATIVE DEBIT: every transaction the swap would broadcast - on EVM an allowance reset and an allowance where they are needed plus the swap, including the L1 data fee where the chain charges one; on Solana every lamport the transaction debits from this wallet, the tip, each account rent this wallet pays and the node's own fee for that exact message - plus a measured reserve for one more transaction afterwards, so a token swap needs native too. \`balance_unavailable\` means a balance could not be READ, which is never the same statement as the wallet being short and always fails closed: retry rather than resize. On Solana only initialized, non-frozen token accounts count as spendable, so a FROZEN holding is present and NOT spendable and is reported apart from the spendable amount. An ineligible quote still returns its full route and still records itself, so a later execute cannot fall back on an older funded quote. Every balance figure here is a QUOTE-TIME OBSERVATION, advisory and ageing: the authoritative read happens again immediately before each signature, so an execute can still refuse for balance after this call said executable. RETURNS on EVM a summary, chain, chainId, tokenIn/tokenOut with address, symbol and decimals, routerAddress, a routeSummary (amountIn/amountOut raw, amountInUsd/amountOutUsd, gasUsd, l1FeeUsd, priceImpact as a fraction, extraFee, routeHops, routePaths) and a safety verdict per leg carrying isHoneypot, isFOT and tax. On Solana it returns a summary, inputToken/outputToken metadata, inputAmountRaw, outputAmountRaw, otherAmountThreshold, slippageBps, priceImpactFraction, routePlan and feePreview - no USD figures at all on that side, and safety is present only when the provider has a verdict. KyberSwap is the primary swap venue. If it cannot route this chain or token pair, quote SwapQuoteUniswap instead; the failure message says when switching venue is the right move.`,
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    description:
      `Execute a REAL on-chain token swap (spends funds, broadcasts a signed transaction). EVM chains route to KyberSwap ONLY; chain "solana" routes to Jupiter. REQUIRES a fresh matching SwapQuote FIRST on the SAME venue - the execute gate blocks a swap that has no fresh matching quote, so always preview with SwapQuote before calling this. EVM tokens must be a CONTRACT ADDRESS or native ETH/native; EVM symbol resolution is disabled to avoid wrong-contract matches. ${TOKEN_FIND_EVM_IDENTITY_RULE} Solana accepts a symbol or mint. \`amountIn\` is the HUMAN decimal of tokenIn (e.g. "1.5", not wei/lamports). For EVM, restricted approval shows chain, token addresses, human \`amountIn\`, and the quote/floor with its quote-time contract-read output symbol. It omits decimals and atomic input and does not independently re-read metadata. The card also carries a QUOTE-TIME spendability line - the source-token requirement and the total native debit including every fee leg and the reserve - labelled as a quote-time observation and re-read before signing; it is disclosure, not the check, and it is not a live balance. EVM execution re-reads both token contracts and refuses unreadable metadata before signing; the card is not proof. Full permission shows no card. BALANCE IS RE-READ HERE AND THAT READ IS THE AUTHORITY, not the quote's: immediately before EVERY signature Vex reads the wallet again - on EVM the input-token and native balances at the pending block, on Solana the spendable input balance (frozen token accounts do not count), the lamports, the node's fee for that exact message, the tip and every account rent this wallet pays - and requires them to cover that transaction, every transaction still authorized after it, and a measured follow-up reserve. A wallet that no longer covers them is refused with NOTHING signed and nothing broadcast, and the refusal distinguishes a wallet that is SHORT from a balance that could not be READ. A quote that was executable minutes ago is not a guarantee here. RETURNS, on a confirmed EVM swap, a summary with chain, chainId, txHash, tokenIn, tokenOut, the DECODED amountIn and amountOut in human units, a status of confirmed or confirmed_unrecorded, and a deliveryCheck when one was made. Every other EVM outcome is a sentence, and they differ: reverted on-chain, confirmed but the amounts could not be decoded yet, refused before signing, and broadcast but NOT yet confirmed - that last one says do not retry and names the ChainRead tx_receipt call you can make yourself. On Solana there is no JSON success arm at all: a broadcast answers "Swap broadcast (signature ...) - confirmation pending, tracked automatically. Do not retry.", and terminality is settled by a background sweep, so never read that sentence as a completed swap. Failed and pending attempts are recorded and shown with chain + tx hash + explorer link, same as confirmed ones.`,
    parameters: {
      type: "object",
      properties: SWAP_SCHEMA_PROPERTIES,
      required: SWAP_SCHEMA_REQUIRED,
  ...
  },
  {
    ...
    actionKind: "read",
    description:
      `Preview a token swap on Uniswap WITHOUT executing. Returns the expected output amount, price impact, the route Uniswap would take, and the fees. EVM ONLY (chain must NOT be "solana") and only on the EVM chains with a verified Vex Uniswap deployment. Tokens must be a CONTRACT ADDRESS or native ETH/native. ${TOKEN_FIND_EVM_IDENTITY_RULE} \`amountIn\` is the HUMAN decimal of tokenIn. Call this BEFORE SwapExecuteUniswap: an execute is authorized only by a fresh matching quote on the SAME venue, so a KyberSwap quote cannot authorize a Uniswap execute. A ROUTE BEING AVAILABLE DOES NOT MEAN THE WALLET CAN PAY FOR IT: this quote also reads the selected wallet and returns an \`eligibility\` block with its own \`eligibilityNote\`, and only \`eligibility.executable\` true authorizes an execute. The three balance outcomes are DISTINCT facts with distinct remedies. \`insufficient_balance\` means the wallet holds less of the input token than the trade needs, and names required, current and missing. \`gas_reserve_insufficient\` means the input side is covered but the wallet cannot cover the TOTAL NATIVE DEBIT: every transaction this swap would broadcast - the allowance legs where they are needed, the swap, and the SEPARATE Vex fee transfer - including the L1 data fee where the chain charges one, plus a measured reserve for one more transaction afterwards, so an ERC-20 swap needs native too. \`balance_unavailable\` means a balance could not be READ, which is never the same statement as the wallet being short and always fails closed: retry rather than resize. \`balanceChecked\` false means no wallet was read and this answer states nothing about funds. An ineligible quote still returns its full route and still records itself, so a later execute cannot fall back on an older funded quote. Every balance figure here is a QUOTE-TIME OBSERVATION at the pending block, advisory and ageing: the authoritative read happens again immediately before each signature, so an execute can still refuse for balance after this call said executable. KyberSwap is the primary swap route - reach for this one when KyberSwap cannot serve the pair or its quote failed for a routing reason.`,
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    actionKind: "user_wallet_broadcast",
    description:
      `Execute a REAL on-chain token swap on Uniswap. SPENDS FUNDS: it signs and broadcasts a transaction from your wallet. REQUIRES a fresh matching SwapQuoteUniswap FIRST on the SAME venue; a KyberSwap quote cannot authorize it. EVM ONLY, and only on the EVM chains with a verified Vex Uniswap deployment. ${TOKEN_FIND_EVM_IDENTITY_RULE} \`amountIn\` is the HUMAN decimal of tokenIn. Restricted approval shows chain, token addresses, human \`amountIn\`, and the quote/floor with its quote-time contract-read output symbol. It omits decimals and atomic input and does not independently re-read metadata. The card also carries a QUOTE-TIME spendability line - the source-token requirement and the total native debit including every fee leg and the reserve - labelled as a quote-time observation and re-read before signing; it is disclosure, not the check, and it is not a live balance. Execution re-reads both token contracts and refuses unreadable metadata before signing; the card is not proof. Full permission shows no card. BALANCE IS RE-READ HERE AND THAT READ IS THE AUTHORITY, not the quote's: immediately before EACH transaction is signed the input-token and native balances are read again at the pending block and must cover that transaction, every transaction still authorized after it (including the fee transfer) and a measured follow-up reserve. A wallet that no longer covers them is refused with NOTHING signed and nothing broadcast, stating required, held and missing, and the refusal distinguishes a wallet that is SHORT from a balance that could not be READ. A quote that was executable minutes ago is not a guarantee here. Returns the transaction hash and the resulting wallet deltas; failed and pending attempts are recorded and shown with chain + tx hash + explorer link, same as confirmed ones. KyberSwap is the primary swap route - reach for this one when KyberSwap cannot serve the pair or its quote failed for a routing reason.`,
    parameters: {
      type: "object",
      properties: UNISWAP_SWAP_SCHEMA_PROPERTIES,
      required: SWAP_SCHEMA_REQUIRED,
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        amountRaw: { type: "string", description: `Amount in raw atomic units of fromToken (e.g. USDC has 6 decimals, so 1 USDC = "1000000"). ${TOKEN_FIND_RAW_AMOUNT_RULE}` },
  ...
  },
  {
    ...
    actionKind: "read",
    description:
      `Preview a cross-chain bridge via Relay WITHOUT executing. Returns the route, the expected destination amount, fees, and the ETA. ${BRIDGE_TOKEN_METADATA_RESULT_DESCRIPTION} Resolve fromToken and toToken separately with TokenFind on their exact chains. ${TOKEN_FIND_EVM_IDENTITY_RULE} ${TOKEN_FIND_RAW_AMOUNT_RULE} \`amountRaw\` is in RAW base units (wei/lamports). Call this BEFORE BridgeExecuteRelay: an execute is authorized only by a fresh matching quote on the SAME venue, so a Khalani quote cannot authorize a Relay execute. Khalani is the primary bridge route - reach for Relay when Khalani does not cover the route (it is the only venue for Robinhood Chain) or its quote failed for a routing reason.`,
    parameters: {
      type: "object",
      properties: {
        fromChain: { type: "string", description: "Source chain ID or alias." },
