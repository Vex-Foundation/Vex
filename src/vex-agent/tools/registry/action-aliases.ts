...
export const ACTION_ALIAS_TOOLS: readonly ToolDef[] = [
  ...
  },
  {
    ...
    actionKind: "read",
    description:
      "Safety-check an EVM token before trading it: detects honeypots and fee-on-transfer (tax) tokens via KyberSwap. Pass the chain and the token contract `tokenAddress` (resolve it with TokenFind first). The former key `address` is retired and is rejected by name. Read-only.",
    parameters: {
      type: "object",
      properties: {
