...
const MARKET_PARAM = {
  ...
  required: true,
  description:
    "The Pendle MARKET (LP) CONTRACT ADDRESS - the market IS its LP token. Find it with pendle__markets_discover or read it from pendle__market_get.",
};
...
export const PENDLE_LP_DUAL_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    lifecycle: "active",
    description:
      "Remove Pendle liquidity into TWO outputs at once: a plain token AND the market's principal token (PT). Burns the LP token (the market address IS the LP token) and delivers both legs to your own wallet, each protected by its own minimum - use it when you want to take part of an LP position back as an ordinary token while keeping principal exposure, instead of exiting entirely to one token with pendle__lp_remove. Needs: the chain, the market address, and amountIn (the LP amount) in human-readable units (e.g. '1.5', not raw base units); tokenOut is optional and defaults to the market's underlying asset. Works BEFORE and AFTER the market's expiry - removal is legal once a market has matured. Returns the transaction hash plus the RECEIPT-DECODED amounts for BOTH legs beside the quoted ones. Approval-gated; pins the canonical Pendle Router. CALL IT TWICE: first with dryRun: true, which quotes and records the authorization; then with the EXACT same params to broadcast. Without that fresh dry run the execute is refused. CANNOT: exit to a single token (use pendle__lp_remove), choose which PT you receive (it is always the market's own), convert the LP straight to PT only, guarantee an exact output amount on either leg, deliver native currency, or send the proceeds anywhere but your own wallet.",
    mutating: true,
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "Add liquidity to a Pendle market with ONE token and KEEP the yield token (YT) it produces, receiving TWO instruments: the market's LP token and its YT. A plain pendle__lp_add sells that YT into the pool; this variant hands it to you, so use it when the user wants LP exposure without giving up the yield leg. It is still a SINGLE-token deposit - Pendle has no two-token 'dual add'. Needs: the chain, the market address, the payment token address (ERC-20 only; pass wrapped native, never native), and amountIn in human-readable units (e.g. '1.5', not raw base units). The market must NOT have expired: liquidity cannot be added after maturity, and a matured market is refused by name. Returns the transaction hash plus the RECEIPT-DECODED amounts for BOTH legs beside the quoted ones. Approval-gated; pins the canonical Pendle Router. CALL IT TWICE: first with dryRun: true, which quotes and records the authorization; then with the EXACT same params to broadcast. Without that fresh dry run the execute is refused. CANNOT: deposit two tokens, sell the YT for you (use pendle__lp_add), buy a YT on its own (pendle__yt_buy), add to an expired market, guarantee an exact output amount on either leg, accept native currency, or send the LP or YT anywhere but your own wallet.",
    mutating: true,
    actionKind: "user_wallet_broadcast",
    params: [
      CHAIN_PARAM,
