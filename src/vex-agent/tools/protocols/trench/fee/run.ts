/**
 * Running the Vex fee leg — AFTER the trade or launch confirmed, and never
 * before.
 *
 * THE INVARIANT (copied from `src/tools/bridge-fee/index.ts:16-18`, which says
 * it about bridges and means it identically here):
 *
 ...
 *
 *   - A REVERTED or AMBIGUOUS trade/launch means this function is never called
 ...
 *     trade failed because its fee did not land. That is missed Vex revenue and
 *     nothing more.
 ...
 * to tell them apart.
 */
export interface TrenchFeeCollection {
  readonly collection:
    | "confirmed"
...
export interface RunTrenchFeeLegInput {
  ...
  /** Anchor on the block the trade/launch confirmed in. */
...
export async function runTrenchFeeLeg(input: RunTrenchFeeLegInput): Promise<TrenchFeeCollection> {
  ...
  try {
    ...
    const outcome: StagedBroadcastOutcome = await signStageBroadcast(
      ...
      plan.txParams,
      {
        ...
        onAccepted: async () => {
          ...
          if (!res.applied) logger.warn("trench.fee.broadcast_accept_miss", { id: feeRowId });
        },
      },
      input.priorLeg,
    );
    ...
    if (outcome.kind === "ambiguous") {
      ...
      logger.info("trench.fee.ambiguous", { id: feeRowId, stage: outcome.stage });
      // Migration 067: the fee row's OWN pending reason — a fee that did not
      // confirm says nothing about whether the trade did.
      await noteHandlerPendingReason("trench.fee", feeRowId, "fee_broadcast_ambiguous");
      return {
    ...
    return {
      collection: await confirmFeeRow(feeRowId, outcome.txHash, plan.feeWei),
    ...
    logger.warn("trench.fee.leg_failed", { id: feeRowId, error: err instanceof Error ? err.name : "unknown" });
    return {
 ...
 * its own arithmetic on the user's own ETH leg, and there is no other party's
 ...
 */
function assertFeeValueAuthorized(chainId: number, plan: TrenchFeeLegPlan): void {
 ...
 * On a SELL the planned amount came from the quote and the signed one is 25 bps
 * of the ETH that actually arrived; recording the planned figure as settled
...
async function confirmFeeRow(
  ...
  try {
    ...
    logger.warn("trench.fee.confirm_cas_miss", { id: feeRowId, rowStatus: result.row.status });
    return "confirmed_unrecorded";
  } catch (err) {
    logger.warn("trench.fee.confirm_failed", { id: feeRowId, error: err instanceof Error ? err.name : "unknown" });
    return "confirmed_unrecorded";
  }
}

/** The fee row exists but the action never reached the point where it is taken. */
export function trenchFeeNotAttempted(reason: string): TrenchFeeCollection {
  return {
    collection: "not_attempted",
...
export function trenchFeeNotCharged(): TrenchFeeCollection {
  return {
