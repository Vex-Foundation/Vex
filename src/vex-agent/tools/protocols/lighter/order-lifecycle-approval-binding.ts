...
export async function assertLighterModifyOrderApprovalBinding(input: {
  ...
  if (
    ...
    || Object.keys(critical ?? {}).sort().join(",") !== [...MODIFY_CRITICAL_KEYS, ...Object.keys(lighterOrderFeeCriticalArgs(input.intent.integratorFees))].sort().join(",")
    || Object.entries(lighterOrderFeeCriticalArgs(input.intent.integratorFees)).some(([key, value]) => critical?.[key] !== value)
...
export async function assertLighterClosePositionApprovalBinding(input: {
  ...
  if (
    ...
    || Object.keys(critical ?? {}).sort().join(",") !== [...CLOSE_POSITION_CRITICAL_KEYS, ...Object.keys(lighterOrderFeeCriticalArgs(input.intent.integratorFees))].sort().join(",")
    || Object.entries(lighterOrderFeeCriticalArgs(input.intent.integratorFees)).some(([key, value]) => critical?.[key] !== value)
