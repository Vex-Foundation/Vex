...
const ORDER_PRICE_PARAM: ProtocolParamDef = {
  ...
  required: true,
  description:
    "Exact human-decimal order price. For market it is the immediate worst acceptable price. For plain limit it is the limit price. For stop-loss or take-profit it is the hard execution bound after the trigger. For stop-loss-limit or take-profit-limit it is the limit price activated after the trigger, which may rest and may never fill. It is never the trigger price itself.",
};
...
const TRIGGER_PRICE_PARAM: ProtocolParamDef = {
  ...
  type: "string",
  description:
    "Required for stop-loss, stop-loss-limit, take-profit, and take-profit-limit orders and forbidden for ordinary market and limit orders. Human decimal trigger price. Vex verifies it is on the protective side of the live market for the current perpetual position.",
};
...
const ORDER_TYPE_PARAM: ProtocolParamDef = {
  ...
  description:
    "Optional native Lighter order type. Use market for an immediate market order; limit for a priced order; stop-loss or take-profit for a market-trigger protective order; and stop-loss-limit or take-profit-limit for a trigger that places a priced limit order. Every stop-loss and take-profit variant is perpetual-only, reduce-only, and requires triggerPrice. Limit, stop-loss-limit, and take-profit-limit support immediate-or-cancel, good-till-time, and post-only. TWAP and entry-with-attached-protection remain unavailable.",
};
...
const ORDER_EXPIRY_PARAM: ProtocolParamDef = {
  ...
  type: "number",
  description:
    "Exact order expiry as a JavaScript epoch-milliseconds integer. Use orderExpiryOffsetMinutes instead when the user says a relative expiry such as '30 minutes from now'. Vex requires the final expiry to be between 5 minutes and 30 days from preview time and requires at least 5 minutes to remain at actual submission for every order whose signed wire expiry is non-nil.",
};
...
export const LIGHTER_READ_TOOLS: readonly ProtocolToolManifest[] = [
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "List Lighter public markets/order books on Core or Robinhood Chain. Use when the user needs symbols, market ids, active/inactive status, spot versus perp coverage, fee strings, minimum order amounts, quote limits, or decimal metadata before requesting detail, depth, trades, or candles. Returns deterministically ordered, bounded market rows plus provider row count, page, nextPage, and truncation disclosure so broad market lists are walkable instead of silently shortened. For a market's default and maximum leverage call lighter__market_get, which derives both from its margin fractions. Read-only: no account, key, signing, order, deposit, or withdrawal access.",
    mutating: false,
    actionKind: "read",
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "Get detailed public metadata for one Lighter market on Core or Robinhood Chain by numeric marketId. Use when the user already has a market id and needs last trade price, daily activity, volume, open interest, fees, decimals, funding fields, minimums, and status before inspecting depth, trades, or candles. Returns matching detail rows and fails clearly if the market id is not found. margin carries defaultLeverage and maxLeverage derived from the market's own fractions; the account's current leverage is a user setting on its position row, not a market property. Base and quote asset ids are numeric provider identifiers, not symbols; never infer labels for them from the market symbol. Read-only: no account data, signing, order placement, deposit, or withdrawal support.",
    mutating: false,
    actionKind: "read",
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "Use only to preview and prepare one native Lighter one-cancels-the-other group that protects an existing perpetual position with exactly two same-size reduce-only children: one stop-loss and one take-profit. Explicit trigger and hard execution-bound prices for both legs, exact side, size, and expiry are required. Vex verifies the live position, persists both child identities, and shows one approval card. After approval it submits one native grouped transaction and calls protection active only when both exact children are proven from authenticated account evidence. Returns status, previewId, matchHash, expiresAt, the exact two-leg preview, approvalReady, userGuidance, and safety; when managed trading is ready it also returns the approval follow-up. It never emulates sibling cancellation, creates an entry order, automatically retries uncertainty, or supports OTO/OTOCO here.",
    mutating: false,
    // NOT `read`, despite preparing nothing on the exchange: the handler writes
    // a durable preview row and, when managed trading is ready, prepares the
    // `lighter.order.create` approval. `approval_prepare` is the kind
