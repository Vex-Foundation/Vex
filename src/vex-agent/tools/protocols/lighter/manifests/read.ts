...
const ONBOARDING_REQUIRED_COLLATERAL_PARAM: ProtocolParamDef = {
  ...
  type: "string",
  description:
    "Optional intended position collateral in human decimals (USDC, 6 decimals), for example \"11\". When provided, the onboarding plan includes the funding legs needed to reach it; when omitted, the plan reports account/key state and only a trading-key registration leg if one is missing.",
};
...
export const LIGHTER_READ_TOOLS: readonly ProtocolToolManifest[] = [
  ...
  },
  {
    ...
    lifecycle: "active",
    description:
      "Check and reconcile the durable state of this Vex wallet's Lighter deposit intents. Use after an approval, a pending or ambiguous Ethereum outcome, or when a second deposit is refused because an earlier intent is unresolved. Phase 1 repair reads only already-staged Ethereum transaction receipts and may advance local state through hash-bound guarded updates; it does not infer credit from generic Lighter account existence. Returns approval state, execution state, staged transaction hashes, any previously proven account index, repair reports, and explicit next-action guidance. An optional intent id checks one local intent; omitting it checks this wallet's unresolved deposit intents. This tool never signs, broadcasts, retries, or replaces a transaction and moves no funds.",
    mutating: false,
    actionKind: "read",
    params: [
      ENVIRONMENT_PARAM,
