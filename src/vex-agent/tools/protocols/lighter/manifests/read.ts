...
const ONBOARDING_REQUIRED_COLLATERAL_PARAM: ProtocolParamDef = {
  ...
  type: "string",
  description:
    "Optional intended position collateral in human decimals (USDC, 6 decimals), for example \"11\". Omit during initial plain-language setup discovery; Vex uses the 1 USDC activation minimum to detect whether funding is required, then asks the user only for their desired deposit amount. Never ask the user for account or API-key indexes.",
};
...
export const LIGHTER_READ_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    lifecycle: "active",
    description:
      "Report whether the selected Vex wallet is ready to trade on Lighter Core and the minimal managed setup steps still required. Use when: the user asks to set up Lighter, trade Lighter perps, check readiness, or continue onboarding. Omit walletAddress to resolve the selected wallet automatically, and never ask a normal user for wallet, account, API-key index, nonce, fingerprint, or key material. Read-only: it checks Ethereum-mainnet USDC, the wallet-owned Lighter account and collateral, and registered Vex trading access. When amountIn is omitted, it uses the 1 USDC activation minimum so an unfunded/new account produces a deposit leg and the assistant can ask only how much USDC the user wants to deposit. Returns account and funding state, whether active local trading access is proven, the minimal ordered plan, and plain-language guidance. An already funded and keyed account returns ready with no legs. Core only in this release. This moves no funds and signs nothing; Vex control is confirmed only by the managed execution path.",
    mutating: false,
    actionKind: "read",
