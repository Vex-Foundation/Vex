...
export const PENDLE_NAVIGATION: ProtocolNamespaceNavigation = {
  ...
  declaration: {
    identity: "Pendle is a term-yield venue where each market splits a yield-bearing asset into a principal token and a yield token with one maturity date.",
    ...
    whenItApplies: "Use it to lock a fixed rate until expiry, take variable yield exposure through a yield token, manage single-token liquidity, move Pendle liquidity, extend a maturity, inspect a matured market, or unwind a term position.",
    characteristicAndLimits: "Every position has an expiry. Principal tokens commit funds until maturity, yield tokens decay to zero at expiry, and early exits are market-priced. Liquidity positions are not fixed-rate locks. Display marks and order-book depth are not executable quotes, speculative points are not yield, and thin markets can have high exit impact.",
    retrievalTerms: [
      "fixed-yield markets",
      "implied APY",
      "principal token",
      ...
      "move Pendle liquidity",
      "standardised yield",
      "price candles",
      "resting orders",
      "dollar price marks",
      "Pendle positions",
      "accrued interest and rewards",
    ],
    facets: [
      "Yield markets",
      "PT trading",
      "YT trading",
      "Mint and redeem (PT + YT)",
      "Liquidity (LP)",
      "Dual-leg liquidity",
      "Move a position (term mobility)",
      "SY wrap and unwrap",
