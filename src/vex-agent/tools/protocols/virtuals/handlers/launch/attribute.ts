 ...
 *     file. The message builder is the other half, and it is launchpad-neutral
 *     because the registry is: `@vex-agent/agentscan/attest-message.js`, the
 *     one module the guard admits as owner of the canonical attest literal.
 *
...
export async function signAndStoreVirtualsAttestation(input: {
  ...
  readonly launchpad: AgentscanAttestLaunchpad;
}): Promise<VirtualsAttestOutcome> {
  let signature: string;
  try {
    signature = await input.signer.signMessage({
      message: buildAgentscanAttestMessage(input.chainId, input.tokenAddress),
    });
  } catch (err) {
