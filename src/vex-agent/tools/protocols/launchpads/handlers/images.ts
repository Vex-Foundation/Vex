/**
 * `launchpads.images` handler - READ-ONLY listing of the user's image locker.
 *
 * METADATA ONLY, and that is a rule rather than an optimisation (rule 07):
 * the bytes live in the desktop app's own store under userData, they are never
 * serialised into a tool result, and nothing here can reach them. The model
 * chooses an `imageId`; the BYTES are resolved later, on the publishing or
 * signing path, through the resolver seam
 * (`shared/launch-image-byte-resolver.ts`), where the recorded digest is
 * verified against the authorization record before anything is committed.
 *
 * THE DIGEST IS NOT IN THE REPLY. It is a content fingerprint of the user's own
 * file and buys the model nothing; `publicUrl` is different and IS included,
 * because a picture that already has a public address does not need publishing
 * again and the agent can only know that if it is told.
 *
 * The metadata itself is an ordinary DB-backed read through
 * `db/repos/launch-images.ts` - deliberately NOT a new
 ...
 * words it can pass straight to the user, that a human has to add an image on
 * the right side of the app before any launch can proceed. Returning `fail`
 * here would read to the model as "the tool is broken, try something else",
 * when the correct behaviour is to stop and ask.
 */
...
interface LockerImageView {
  ...
  height: number;
  uploadedAt: string;
  /** The public content-addressed URL, or null when this picture is not published. */
  publicUrl: string | null;
}

const EMPTY_LOCKER_GUIDANCE =
  "The image locker is empty. A token launch requires an image and you cannot upload one - "
  + "ask the user to add a picture on the image card on the right side of the app, then check "
  + "again. Do not attempt a launch until an image is available.";

export async function launchpadsImagesHandler(p: Record<string, unknown>) {
  ...
  try {
    ...
    return fail(
      `The image locker could not be read (${launchpadsFailureDetail("launchpads__images_list", err)}). `
  ...
  const images: LockerImageView[] = rows.slice(0, limit).map((row) => ({
    imageId: row.imageId,
    label: row.label,
    mime: row.mime,
