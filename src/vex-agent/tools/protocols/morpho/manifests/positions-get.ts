...
export const MORPHO_POSITIONS_GET_TOOL: ProtocolToolManifest = {
  ...
  description:
    ...
    + "ONE WALLET PER CALL, by design. Morpho's API accepts a list of addresses and this tool refuses to send one, "
    + "because a position read maps an account to its debts. A second address is rejected by name. "
    ...
    + "`priceDropToLiquidationPercent`. A NULL HEALTH FACTOR MEANS NO DEBT, NOT SAFETY: a supply-only position has "
    ...
    + "warnings), collateral, supplied and borrowed each as {raw, decimals, symbol, human, usd}, share balances, "
    + "health factor and band, price move to liquidation, margin and borrow PnL as SIGNED amounts, borrow return "
    + "percent. Per vault position: the vault (address, v1 or v2, name, chain, listed), asset, deposit as {raw, "
    + "decimals, symbol, human, usd}, shares, PnL, roe and the vault APY. Plus USD portfolio totals and a riskFlags "
    ...
    + "positions, so this tool finds candidates from the wallet's own V2 transaction history, reads each, and "
    + "reports the sweep in `vaultV2Coverage`; when `complete` is false a V2 position may exist that is not listed. "
    + "Set `includeVaultV2` false to skip it. "
