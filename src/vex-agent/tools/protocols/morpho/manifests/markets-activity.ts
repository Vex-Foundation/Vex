...
export const MORPHO_MARKETS_ACTIVITY_TOOL: ProtocolToolManifest = {
  ...
  description:
    ...
    + "the other's scale is a thousand-billion-fold error. `shares` are accounting units, carry no decimals, are not "
    + "comparable between markets and must never be shown as money. "
    + "FILTERS AND PAGING. Every filter that ran is echoed in `filtersApplied`, and a value outside the accepted set "
    + "is REJECTED BY NAME rather than dropped. `since` and `until` are unix SECONDS; a milliseconds value is "
    + "refused by name because it would select a window far in the future and return an empty history that reads as "
    + "a dead market. Morpho ranks and pages server-side over all matches, so `matched` is exact and `hasMore` comes "
  ...
  params: [
    ...
    },
    {
      ...
      type: "string",
      acceptsStringArray: true,
      enum: MORPHO_ACTIVITY_TYPE_KEYS,
      description:
        `Comma-separated event types, or an array, one of: ${MORPHO_ACTIVITY_TYPE_KEYS.join(", ")}. Omit for every `
