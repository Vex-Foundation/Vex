...
export const LEND_BORROW_TOOLS: readonly ProtocolToolManifest[] = [
  ...
  },
  {
    ...
    actionKind: "read",
    params: [
      { key: "address", type: "string", description: "Wallet address. Defaults to the session's selected Solana wallet." },
  ...
  },
  {
    ...
    namespace: "solana",
    lifecycle: "active",
    description: "Open, adjust, or close a Jupiter Lend Borrow position in ONE call: deposit/withdraw collateral and/or borrow/repay debt, via the six params below (at least one required). Approval preview shows the vault's max LTV / liquidation threshold and an estimated post-operation LTV before you approve. Reads first: solana.lend.borrowVaults for vaultId + thresholds, solana.lend.borrowPositions for an existing positionId. Jupiter's /operate endpoint never wraps or unwraps native SOL — depositing or repaying a leg denominated in native SOL requires WRAPPED SOL (WSOL) already sitting in your wallet; this call fails clearly beforehand if that balance is insufficient, rather than broadcasting. This tool broadcasts and returns truthful-pending — it is tracked automatically, do not resubmit.",
    mutating: true,
    ...
    params: [
      ...
      { key: "depositAmount", type: "string", description: "Add this many raw atomic units of the vault's collateral token (scale = the vault's supplyTokenDecimals). Mutually exclusive with withdrawAmount/withdrawAll." },
      { key: "withdrawAmount", type: "string", description: "Remove this many raw atomic units of collateral (scale = the vault's supplyTokenDecimals). Mutually exclusive with depositAmount/withdrawAll." },
      { key: "withdrawAll", type: "boolean", description: "Withdraw ALL collateral from this position. Mutually exclusive with depositAmount/withdrawAmount." },
      { key: "borrowAmount", type: "string", description: "Borrow this many raw atomic units of the vault's debt token (scale = the vault's borrowTokenDecimals). Mutually exclusive with repayAmount/repayAll." },
      { key: "repayAmount", type: "string", description: "Repay this many raw atomic units of debt (scale = the vault's borrowTokenDecimals). Mutually exclusive with borrowAmount/repayAll." },
      { key: "repayAll", type: "boolean", description: "Repay ALL debt on this position, including accrued-interest dust. Mutually exclusive with borrowAmount/repayAmount." },
    ],
    exampleParams: { vaultId: 1, depositAmount: "30000000" },
    requiresEnv: "JUPITER_API_KEY",
