...
export const PREDICT_TOOLS: readonly ProtocolToolManifest[] = [
  ...
  },
  {
    ...
    namespace: "solana",
    lifecycle: "active",
    description: "Browse the GLOBAL Jupiter Prediction trade feed - every trader, every market, newest activity as the venue publishes it. Use this when the question is what the venue as a whole is trading; a wallet's own fills are solana__predict_trade_history_list. Returns `data`, one row per trade, with amountUsd and priceUsd as exact-decimal USD strings each carrying a raw *Micro sibling and every other field (ids, side, action, titles) passed through, plus a `pagination` block of start, end, total and hasNext. The feed has NO upstream scope or paging of its own, so the whole feed is fetched and Vex applies your window client-side: limit is REQUIRED (integer 1-100) precisely so an unbounded response is never returned by accident, and an out-of-range limit or offset is REJECTED by name rather than clamped.",
    mutating: false,
    actionKind: "read",
    params: [
