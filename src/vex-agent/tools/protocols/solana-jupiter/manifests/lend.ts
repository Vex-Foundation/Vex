...
export const LEND_TOOLS: readonly ProtocolToolManifest[] = [
  ...
  },
  {
    ...
    namespace: "solana",
    lifecycle: "active",
    description: "Withdraw tokens from a Jupiter Lend EARN vault — simple lending, not a Borrow collateral withdrawal (for that, use solana.lend.borrowOperate's withdrawAmount/withdrawAll instead). Broadcasts and returns truthful-pending: confirmation is tracked automatically, do not resubmit. Read solana.lend.positions for the raw atomic-unit balance available to withdraw.",
    mutating: true,
    actionKind: "user_wallet_broadcast",
    params: [
      { key: "asset", type: "string", required: true, description: "Token address to withdraw." },
      { key: "amount", type: "string", required: true, description: "Amount in raw atomic units of the asset (e.g. USDC has 6 decimals, so 1 USDC = \"1000000\") — see solana.lend.positions for your current balance." },
    ],
    exampleParams: { asset: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", amount: "1000000" },
    requiresEnv: "JUPITER_API_KEY",
