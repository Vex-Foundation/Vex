...
export const LAUNCHPADS_IMAGE_PUBLISH_DISCOVERY = {
  "launchpads.image_publish": {
    embeddingText: embeddingText(
      `Submit one staged picture from the user's image locker to Vex's public image host, a content-addressed host, so a token launch can put its address on chain. Use this when a launch needs a public link. The bytes become PUBLIC: anyone with the link can fetch them until the user withdraws them. The address is the sha256 hash of the exact bytes, so it can never point at a different picture. Publishing the same picture twice returns the same address and uploads nothing. This launches nothing and spends no gas. Example queries: publish my launch image, get a public link for the token picture, host the image for the launch.`,
    ),
    aliases: [
