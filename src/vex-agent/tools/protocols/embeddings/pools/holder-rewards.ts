...
export const POOLS_HOLDER_REWARDS_DISCOVERY = {
  "pools.holder_rewards": {
    embeddingText: embeddingText(
      `Read the fees to holders state of a coin on pools.fun, on Robinhood Chain: which contract pays those fees and what it owes one wallet. `
      + `Use when a coin is marked as sharing its fees with holders, when the user asks what holding it has earned them, or when its paying contract and reward mode must be confirmed. `
      + `Nothing is staked: the payout follows from holding the coin. `
      + `A coin that does not share its fees is reported as such, a choice fixed at creation. `
      + `Nothing is signed. `
      + `Example queries: what have I earned holding this coin, does this coin pay its holders.`,
    ),
    aliases: [
      "fees to holders",
