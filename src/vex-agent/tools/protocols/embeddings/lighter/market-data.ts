...
export const LIGHTER_MARKET_DATA_DISCOVERY = {
  ...
  "lighter.order.preview": {
    embeddingText: embeddingText(
      `Create a live Lighter order preview: market, limit, stop-loss, take-profit, and the -limit variants. ` +
      `Use when the user requests a market order, priced limit, maker-only limit, or perpetual protection. An ordinary limit with no stated behavior defaults to keep open (good-till-time); preserve explicit IOC, GTT, post-only, or duration choices. Trigger-limit orders require an explicit choice; market, stop-loss, and take-profit require IOC. Protection requires reduce-only, an exact trigger, price bound, side, and size. ` +
      `Returns exact preview terms and an approval card; never signs without approval. Example queries: buy 0.1 ETH at 3000 on Lighter; prepare trade approval; keep my limit bid open; GTT stop-loss-limit; GTT take-profit-limit.`,
    ),
