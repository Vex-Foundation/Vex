...
export const POOLS_HOLDER_REWARDS_CLAIM_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    description:
      ...
      + "WHAT IT PAYS depends on the reward mode the token was launched with, which is read from the DistributorDeployed event the suite's own HolderRewardsDeployer emitted for this token, never from the launchpad's row: token mode pays the launched token, paired mode pays the asset the pool is paired against, and both mode pays both. Every leg is reported separately with its RAW base-unit amount, its decimals and its asset address, plus a scaled figure where the asset answered decimals(); never add two legs together and never assume 18 decimals. Some distributors have no paired leg at all, and that is reported as the ABSENCE of a leg rather than a zero. WHETHER A LEG EXISTS is decided by what the simulated claim() itself returns, never by the separate earned/earnedPaired accrual reads, so a leg the transaction will pay is still reported when those optional reads do not answer. Each leg carries the distributor's own earned figure at the same block where that read answered, which differs from the simulated payout by the seconds between the two reads because the reward streams continuously; where it did not answer, earnedRaw is null and the leg says so rather than showing a zero. "
