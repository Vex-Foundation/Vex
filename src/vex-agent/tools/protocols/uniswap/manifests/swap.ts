...
export const UNISWAP_SWAP_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    namespace: "uniswap",
    lifecycle: "active",
    description: "Get the best Uniswap route across V2 + V3 — output amount, route, price impact, gas, and token-safety signals (factory allowlist, liquidity, fee-on-transfer). A HIDDEN fallback for KyberSwap, available after an eligible KyberSwap route-not-found failure, or the Kyber swap transaction reverting on-chain, reveals it for this session. Read-only, no execution. Vex charges 25 bps (0.25%) on the input token: the quoted output is for amountIn MINUS that fee (see swapAmountRaw and the vexFee block), while amountIn stays the total debited. The rate and receiver are fixed — passing fee, feeBps, feeReceiver or feeAmount is rejected by name.",
    mutating: false,
    actionKind: "read",
    params: [
