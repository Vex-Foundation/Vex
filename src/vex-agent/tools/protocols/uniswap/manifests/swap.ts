...
export const UNISWAP_SWAP_TOOLS: readonly ProtocolToolManifest[] = [
  {
    ...
    namespace: "uniswap",
    lifecycle: "active",
    description: "Get the best Uniswap route across V2 + V3. Returns the output amount, the route, price impact, gas, and token-safety signals (factory allowlist, liquidity, fee-on-transfer). Read-only, no execution. Covers the EVM chains with a verified Vex Uniswap deployment; pass token ADDRESSES. Vex charges 25 bps (0.25%) on the input token: the quoted output is for amountIn MINUS that fee (see swapAmountRaw and the vexFee block), while amountIn stays the total debited. The rate and receiver are fixed - passing fee, feeBps, feeReceiver or feeAmount is rejected by name. KyberSwap is Vex's primary swap route. Use this when KyberSwap cannot serve the pair, or when its quote failed for a routing reason. A ROUTE IS NOT AN OFFER: this quote also reads the selected wallet and reports whether it can actually pay, in the eligibility block and eligibilityNote. eligibility.executable false means this quote authorizes NO execute, and the three balance reasons are distinct: insufficient_balance (the wallet holds less of the input token than the trade needs - required, current and missing are stated), balance_unavailable (a balance could not be read, so nothing is known and it fails closed - retry rather than resize), gas_reserve_insufficient (the wallet cannot cover the NATIVE debit, which is every transaction this swap would broadcast - allowance legs, the swap, the separate Vex fee transfer - plus a measured reserve for the next move, so an ERC-20 swap needs native too). balanceChecked false means no wallet was read and this answer states nothing about funds. Those figures are a QUOTE-TIME observation at the pending block, advisory only: the authoritative read happens immediately before each transaction is signed, and it refuses there if the wallet no longer covers the remaining legs.",
    mutating: false,
    actionKind: "read",
    params: [
