...
const TRADES_PARAMS: readonly ProtocolParamDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      + "first trade time). full adds `pairObservedRetainedAmount` (the amount this wallet retains of what it BOUGHT on THIS pool as the venue observed it, never an on-chain balance and never divisible by token supply, with that refusal carried as a field beside it) and base-token volumes. none drops the "
...
export const DEEP_DIVE_TOOLS: readonly ProtocolToolManifest[] = [
...

  {
    ...
    description:
      ...
      + "THE PER-WALLET AMOUNT IS `pairObservedRetainedAmount` AND IS NEVER DIVIDED BY TOKEN "
      + "SUPPLY. It is what a wallet retains of what it BOUGHT ON THIS POOL as the venue "
      + "observed it, not an on-chain balance: transfers, other pools and other venues are "
      + "invisible here and this endpoint never reports supply. Each row therefore carries "
      + "`pairObservedRetainedSupplyShare: {status: not_determinable}` and "
      + "`pairObservedRetainedEntityRelationship: unknown` as FIELDS beside the number, and "
      + "`unknowns.supplyShare` and `unknowns.entityRelationship` repeat them at the envelope. "
      + "MEASURED: dividing these amounts by supply made the top 20 wallets of one pool sum to "
      + "about 110 percent of that token's supply. For holder distribution use "
      + "dexscreener__pair_details_get with the holders and supply field groups. The "
      ...
      + "provider's own retained amount (`pairObservedRetainedAmount`) and the pair's current `priceUsd`, with both factors "
      ...
      + "IT DEPENDS ON THE RANKING, MEASURED. The provider sends `pairObservedRetainedAmount` on some sorts and "
      ...
      + "DECIMAL PLACES: `amountBuy`, `amountSell` and `pairObservedRetainedAmount` arrive carrying about 15 to "
