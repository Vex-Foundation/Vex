...
- On-chain EVM forensics (tx receipts, ERC-721 mint detection, and \`erc20_balance\`, a direct \`balanceOf\` for one token and one owner, defaulting to your own wallet): \`chain_read\` — covers Khalani chains AND local chains (\`chain: "robinhood"\` / \`"4663"\`; the param is \`chain\`, and \`chainId\` is refused by name). \`erc20_balance\` asks the token contract itself, so it is what settles "did that buy actually deliver": \`wallet_balances\` reports a scan projection, and receipt Transfer logs are written by the token. (Native balances → \`wallet_balances\`; token symbol/name → \`token_find\`.)
...
- **See a whole namespace at once, then fetch the schemas you want.** \`discover_tools(namespace="x", list=true)\` returns EVERY tool of that namespace as one-line rows (toolId, mutating flag, description — no param schemas), unranked and untruncated. Use it when you need to know what a namespace can do rather than to match one intent. The listing's \`nextStep\` field, at the very top of the result, then tells you how to fetch the full \`params\` schemas of the rows you picked and make those tools callable by name — read it and follow it rather than re-running discovery per tool.
