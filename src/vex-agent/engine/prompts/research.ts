 ...
 * `tool-usage.ts` §6). Holds the `WebResearch` shapes, the per-mode research
 ...
 *
 * This is the SECOND `WebResearch` description surface — the first is the tool
...
function isWebResearchAvailable(): boolean {
  return isInternalToolAvailable("WebResearch");
}
...
function buildTokenResearchMapSection(): string {
  ...
  // The fresh-Solana clause names `solana__tokens_discover`, so it sits behind
  ...
  lines.push(
    ...
    + (isProtocolNamespaceAvailable("solana")
      ? "fresh Solana -> `solana__tokens_discover` category=recent (measured: tokens 10-175 s old, createdAt on every row proves age); "
      : "")
    + "fresh Robinhood -> `trench__tokens_discover` status=curve sort=time (launchpad registry, ~2 s cache, launchedAtMs proves age) - COVERAGE: only tokens launched on Trench Express, never other Robinhood pools. Use DexScreener afterwards, for depth, price sanity and risk once the pool is indexed.",
  );
  ...
  if (isWebResearchAvailable()) {
    lines.push(
      "- `WebResearch` — news, narrative and off-chain context; the only surface that reads the open web. It answers WHY something is moving, never how deep the pool is.",
    );
  }
  if (isInternalToolAvailable("TwitterAccount")) {
    lines.push(
      "- `TwitterAccount` — the project's OWN posts, and cashtag chatter around a token you already found. Always pass an engagement floor and a time window; an unfiltered cashtag sweep is mostly spam.",
    );
  }
...
/**
 * Research layer WITHOUT the `WebResearch` shapes: the tool is not callable in
...
\`WebResearch\` is one tool: it searches through Tavily and reads the pages it finds. Default: ${WEB_SEARCH_DEFAULT_MAX_RESULTS} hits, the top ${WEB_SEARCH_DEFAULT_FETCH_TOP} read in full, one batch call (~12 KB). Pick the smallest shape that answers the question:
