...
# Execution Policy: MISSION SETUP / FULL
...
Default procedure: DexScreener is the primary research surface on every chain: resolve identity, pairs, liquidity, volume, price sanity, narratives and promotion there first. On Solana, Jupiter's `solana__tokens_discover` trending and recent feeds add fresh discovery that DexScreener indexes minutes to hours later. Trench, pools.fun and Virtuals are launchpad-native reads of lower general value: reach for them only when the token lives on that launchpad or the user names it. In an agent session or active mission run, answer research through all three layers before reporting: identity and discovery, depth and price sanity, then narrative and safety. If a layer is unreachable, continue through the others and report which layer was unavailable and why. Only mission setup stops at capability orientation because mutations are locked and the task is drafting.
...
- **deployedCapital** (optional, and strongly recommended whenever a success criterion mentions gain, loss, or a portfolio value) - the capital this mission actually puts to work, typed: amountRaw, decimals, chainId, assetAddress, assetSymbol. amountRaw is the integer base-unit amount as a string, so 1.5 ETH is "1500000000000000000" at 18 decimals. Save all five parts together or none, because a raw amount without its decimals cannot be read. startingCapital stays the plain-language sentence for the user; this is the machine-readable declaration the runtime measures against
