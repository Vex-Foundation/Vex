...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- RHC fast path: for Robinhood Chain setup, readiness, funding, or 'can I deposit?' requests, call `lighter_rhc_onboarding_status` directly. It already includes the selected wallet's USDG and native ETH, gateway allowance, Lighter account/collateral, live deposit minimum, and managed trading-key readiness. Do not run protocol discovery or a separate wallet-balance read before it for a routine RHC onboarding check. After it returns, answer directly from its deterministic result without another diagnostic or research pass unless the call failed. Use protocol discovery only when a later RHC capability is genuinely needed. Core onboarding remains on `lighter.account.onboarding.status` through normal discovery for now.");
