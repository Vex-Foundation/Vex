...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- QUOTE BEFORE ANY DEPOSIT OR WITHDRAWAL, AND A QUOTE COMMITS NOTHING. `morpho.vault.quote` prices one specific amount into or out of one vault: the shares it would mint or burn, the share price the transaction would enforce, what the wallet would have to approve or sign first, the decoded transaction, its gas, and a simulation verdict. Run it before recommending that a user put money into a vault or take it out, and run it whenever the question is how many shares an amount would buy or what would have to be approved first. It signs nothing, sends nothing and approves nothing, so there is no reason to hesitate over one. Pass `walletAddress` to learn which requirements would actually still apply to that wallet; without it the answer is what a fresh wallet would face.");
  lines.push("- READING A QUOTE HONESTLY. `input` and `expectedShares` are in DIFFERENT units, each carrying its own decimals, so never compare or subtract the two raw figures. A DEPOSIT is a bundled transaction with an on-chain share-price ceiling; a WITHDRAWAL is a direct vault call with no bundle, no such ceiling and usually no requirements, and none of those absences is a defect. A deposit simulation that REVERTS before the one-time approval exists is normal and is not a fault in the vault: report it as a missing approval, never as a broken vault. `transport-ambiguous` means the node did not answer, which is not the same as a proven revert, and a null gas figure means the node refused to estimate rather than that the transaction is free.");
  lines.push("- All nine Morpho tools are READ-ONLY, the preview included. Vex cannot supply, borrow, repay, deposit into a vault or withdraw from one, and it cannot add collateral or repay to rescue a position that is about to be liquidated, claim a reward, or grant or revoke an approval. A quote is a priced hypothetical and never a transaction that has been set up: present it as what the user would have to do themselves. When a position is in danger, say what is happening and what the user would have to do elsewhere, rather than implying Vex can act.");
  lines.push("");
