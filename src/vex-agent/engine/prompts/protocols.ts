...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- PLANNING starts at the image locker: read it with `trench.images` (the locker is SHARED by both launchpads) and pass the chosen `imageId`. You can never create or supply an image, and without one the token launches and renders blank on pools.fun forever. Unlike Trench there is NO byte limit here, because pools.fun hosts the image off-chain at its original quality: an image too large for Trench's on-chain budget, which the app badges as pools only, still launches fine on this launchpad.");
