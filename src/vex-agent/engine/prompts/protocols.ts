...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- SEVENTEEN MORPHO TOOLS: ten reads and previews, plus SEVEN that move real funds. On VAULTS: `morpho.vault.deposit` and `morpho.vault.withdraw`. On BLUE MARKETS: `morpho.market.supplyCollateral`, `morpho.market.borrow`, `morpho.market.repay` and `morpho.market.withdrawCollateral`. On REWARDS: `morpho.rewards.claim`. Vex can now run the whole borrowing lifecycle on a market it vouches for, so it CAN act on a position near liquidation: supplying collateral or repaying debt both raise the health factor. Every one of the seven takes `dryRun` for a rehearsal that signs nothing and records its outcome in the activity ledger. SIX of the seven are gated on a fresh matching quote; `morpho.rewards.claim` is NOT, because a claim has no price, no slippage and no size to quote - do not look for a rewards quote tool, there is none.");
  lines.push("- QUOTE BEFORE ANY MARKET OPERATION, PER DIRECTION. `morpho.market.quote` prices one operation on one market - `direction` is supplyCollateral, withdrawCollateral, borrow or repay - and returns the vouching verdict, the health factor BEFORE and AFTER against the floor, the position, the market's free liquidity, the allowance plan and the decoded transaction. It signs nothing. It is MANDATORY before the matching execute, and a quote of ONE direction NEVER authorizes another: a collateral quote cannot authorize a borrow. Every projection belongs to ONE wallet - the one named, else the session's, else a stand-in with no position, which the reply says out loud.");
  lines.push("- THE 1.25 HEALTH-FACTOR FLOOR IS A REFUSAL, NOT ADVICE. Vex REFUSES any market operation that would leave the position below 1.25, projected from freshly accrued state and re-checked immediately before signing, so an operation that was safe when planned is refused rather than sent if the oracle moved. This is why: Morpho has NO CLOSE FACTOR, so one liquidation takes the WHOLE position rather than the underwater part. Tell a user asking for a bigger borrow that the floor refused it and what would make it possible - more collateral, or a smaller amount - rather than implying the market said no.");
  ...
  lines.push("- WHAT VEX STILL CANNOT DO ON MORPHO, deliberately: it cannot CLAIM a reward (that is an on-chain transaction the user must send themselves), and it cannot perform any ATOMIC combination of the operations above. Neither is an oversight and neither is coming for free: say what is happening and what the user must do elsewhere rather than implying Vex can act.");
  lines.push("");
