...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- All six Morpho tools are READ-ONLY. Vex cannot supply, borrow, repay, deposit into a vault or withdraw from one, and it cannot add collateral or repay to rescue a position that is about to be liquidated. When a position is in danger, say what is happening and what the user would have to do elsewhere, rather than implying Vex can act.");
  lines.push("");
