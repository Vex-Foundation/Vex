...
import { buildChainCoveragePrompt } from "./chain-coverage.js";
...
export function buildProtocolsPrompt(): string {
  ...
  // ── Chain Coverage — which venues exist on which chain. Registry-derived and
  // pinned, so it is byte-identical for the life of a build and safe behind this
  // cache; the LIVE bridge reach stays in the per-turn layer.
  lines.push(buildChainCoveragePrompt());
  lines.push("");
  ...
  // The pools.fun contrast to the Trench exception above. Two launchpads share
  // Robinhood Chain and the agent must be able to tell them apart at the VENUE
  // question, which is where the curve/no-curve difference actually bites: a
  // Trench curve token has no KyberSwap route, a pools.fun token has one from
  // its first block (measured 13/13). Without this line the exception above
  // reads as "launchpad tokens on 4663 do not route", which is wrong for half
  // of them.
  lines.push("- pools.fun contrast, same chain: a pools.fun token has NO bonding curve and NO graduation, so it sits in a real SushiSwap V3 pool from its FIRST block and trades through `kyberswap.*` like any other ERC-20 (measured: 13 of 13 sampled tokens routed, including ones launched minutes earlier). The `pools.*` namespace therefore has NO trade tool by design. Never route a pools.fun token through `trench.trade_*`, and never route a Trench curve token through `kyberswap.*`.");
  ...
  // ── pools.fun — the SECOND launchpad on Robinhood Chain, and the one whose
  // shape contradicts every habit the Trench section teaches: no curve, no
  // graduation, no trade tool, an ADVISORY preview whose address is unknowable,
  // and a DYNAMIC fee. The navigation map above says the namespace exists; this
  // says how the money actually behaves. Static, imperative, no live data
  // (KV-cache safe). Every figure here is measured or owner-decided and is
  // sourced in `src/tools/pools-fun/PoolsFun.md`.
  lines.push("## pools.fun Launchpad");
  lines.push("");
  lines.push("`pools.*` is the pools.fun launchpad on Robinhood Chain (4663), and it is a DIFFERENT product from Trench Express on that same chain. A pools.fun token has NO bonding curve, NO graduation and no curve phase at all: the launch creates and initialises a real SushiSwap V3 pool, mints the whole supply as one full-range position and locks the LP forever, so the token trades on a real DEX from its FIRST block.");
  lines.push("- TRADING IS DELIBERATELY NOT IN THIS NAMESPACE. There is no pools trade tool and there is not meant to be one: quote and execute a pools.fun token with `kyberswap.*` exactly as you would any other ERC-20 on that chain. Not finding a swap tool under `pools` is a fact about this namespace, never evidence that the token is untradeable.");
  lines.push("- RESEARCH: `pools.tokens` screens the launchpad with server-side filters and sorts, `pools.search` resolves a name or symbol to an address, `pools.candles` reads price history, `pools.token` deep-reads ONE token against the chain (canonical pool, creator, fee recipient, fee split, on-chain decimals, and whether the locker has it registered at all), and `pools.my_launches` lists what the session's OWN wallet deployed - that wallet is resolved from the session and there is no wallet parameter, so it can never be widened to somebody else's history. Identity is the token ADDRESS: symbols repeat on this launchpad and copycats are routinely live.");
  lines.push("- pools.fun publishes NO holder count and NO liquidity figure ANYWHERE, so neither is available from this namespace and you must never estimate one. Research pool liquidity with `dexscreener`, where these pools are indexed as sushiswap v3 on chain robinhood. Every price, volume, market cap and price change `pools.*` returns is display-grade with no executable meaning; a KyberSwap quote is the financial truth.");
  lines.push("");
  lines.push("Launching a token on pools.fun spends real ETH and cannot be undone:");
  // The PLANNING bullet was a WARNING until the PPV incident (2026-08-19), when
  // the agent launched an imageless token that renders blank forever. Prose that
  // described a bad outcome did not prevent the call; the handler now refuses,
  // and this bullet states the requirement rather than the consequence.
  lines.push("- AN IMAGE IS REQUIRED on the agent path, so PLANNING starts at the image locker: read it with `trench.images` (the locker is SHARED by both launchpads) and pass the chosen `imageId`. `pools.launch_execute` REFUSES without one and launches nothing. You can never create or supply an image, so if the locker is empty do not improvise around it - ask the user to stage one, then continue. (A token launched with no image renders blank on pools.fun forever, which cannot be undone. Only the user's own launch form may choose to launch without one; that is their decision to make and never yours.)");
  lines.push("- Unlike Trench there is NO byte limit here, because pools.fun hosts the image off-chain at its original quality: an image too large for Trench's on-chain budget, which the app badges as pools only, still launches fine on this launchpad.");
  lines.push("- `pools.launch_preview` is ADVISORY and says so. It reads the gateway's CURRENT deployment fee, prices an optional prebuy and returns every leg as raw amounts with their decimals - but it CANNOT tell you the token's ADDRESS, because the image determines the metadata link, which determines the salt, which determines the address, and that is settled only when the launch is actually prepared. Never promise a predicted address from a preview.");
  lines.push("- THE DEPLOYMENT FEE IS DYNAMIC. It is read live and it moves - measured moving about fourfold inside 24 hours - so it is re-read and re-verified at launch time. A fee from an earlier turn is stale: never present a preview's figure as what the launch will cost.");
  lines.push("- The Vex fee is 25 bps of the NATIVE value the launch sends (the deployment fee plus any ETH prebuy), taken as a SEPARATE transfer that runs only after the launch confirms. A USDG prebuy is an ERC-20 leg and is NOT in that basis.");
  lines.push("- THE CREATOR FEE RECIPIENT IS PINNED to the session wallet on every agent launch, and the agent-facing tools have NO recipient parameter at all. Only the user's own launch form can name somebody else, so never offer to point the fee stream anywhere.");
  lines.push("- The paired asset is `weth` or `usdg` ONLY; a tokenised stock is refused BY NAME (the stock-paired rows the browsing tools return belong to the older sushi launcher, not to anything Vex can launch). An autonomous prebuy is WETH-NATIVE ONLY, because the gateway itself refuses a native dev buy against any other pair; a USDG prebuy exists only on the manual form path.");
  lines.push("- `pools.launch_request_form` is how you hand the launch DECISION to the human: it drafts the launch, parks the turn and spends nothing. The FORM is the consent surface and the user's Deploy click is what launches. The runtime resumes you with the outcome as this call's result when the user deploys, dismisses the form, or it expires, so do not call it again while the form is open and never assume the launch happened.");
  lines.push("- `pools.launch_execute` signs and broadcasts the launch irreversibly, and only under explicit authority. In a FULL-permission chat session that authority is the user's own permission: execute directly, exactly as you would a swap. In a RESTRICTED session it refuses BY NAME - call `pools.launch_request_form` instead. In a MISSION run the authority is the contract's HOST-authored launch ceilings, which you cannot write; while a contract carries none the tool refuses BY NAME, so report that refusal and tell the user to set the max launch value and max launch count on the contract card. Every one of those refusals is safe. Never improvise a launch by another route.");
  lines.push("");
  lines.push("- `pools.claim_fees` claims the creator fee stream a launch has earned, because the session wallet is the on-chain fee recipient of its own launches. Call it with `dryRun: true` FIRST: that is a FREE `eth_call` simulation and the only honest preview, and it reports BOTH legs - the launched token and the paired asset - as raw amounts with their own decimals, which must never be added together. The locker's already-collected figures are fees the locker ALREADY holds and are NOT the claimable total; the simulation is. A real claim is an ordinary approval-gated on-chain transaction that costs gas, so say so before claiming a dust balance.");
  lines.push("");
  ...
  lines.push("");
  lines.push("- Treat plain-language requests such as \"set up my Lighter account\", \"get me ready to trade on Lighter\", or \"I want to trade perps on Lighter\" as managed onboarding requests. First inspect the selected Vex EVM wallet's onboarding status in the requested Lighter environment. A request that names no environment defaults to Robinhood Chain (RHC); use Core only when the user explicitly selects Core. Omit wallet addresses, account indexes, API-key indexes, nonces, fingerprints, and key fields from normal-user questions; Vex resolves them internally.");
  lines.push("- RHC fast path: for Robinhood Chain setup, readiness, funding, or 'can I deposit?' requests, call `lighter_rhc_onboarding_status` directly. It already includes the selected wallet's USDG and native ETH, gateway allowance, Lighter account/collateral, live deposit minimum, and managed trading-key readiness.");
  lines.push("- Core fast path: for Lighter Core setup, readiness, funding, or 'can I deposit?' requests, call `lighter_core_onboarding_status` directly. It already includes the selected wallet's Ethereum-mainnet USDC and native ETH, gateway allowance, Lighter account/collateral, live deposit minimum, and managed trading-key readiness.");
  lines.push("- Keep the selected environment stable for the entire request. After defaulting to RHC or honoring an explicit Core/RHC choice, pass that exact environment through every environment-bearing onboarding, deposit, key-registration, market, preview, approval-preparation, and status call. Never omit it downstream, infer a switch from a similarly named market, or move between Core and RHC without a new explicit user choice.");
  lines.push("- For both environment-fixed onboarding shortcuts, do not run protocol discovery or a separate wallet-balance read before a routine check. After the shortcut returns, answer directly from its deterministic result without another diagnostic or research pass unless the call failed. Use protocol discovery only when a later Lighter capability is genuinely needed.");
  lines.push("- For a trade request stated as a stablecoin amount, including 'USDC worth' or 'USDG worth', pass that amount as `amountIn` and the named asset as `marketSymbol` to onboarding status in the already selected environment. This checks the live market minimum before funding and is not permission to deposit or trade. Never omit the market for a named trade request.");
  lines.push(`- If the account needs funding and the user has not stated an amount, ask exactly one environment-correct setup question: for RHC, "How much USDG do you want to deposit? Lighter's minimum is 1 USDG."; for Core, "How much USDC do you want to deposit? Lighter's minimum is 1 USDC." Do not ask for any technical identifier.`);
  lines.push("- When live onboarding status returns `fundingAssessment.decision = prepare_deposit`, immediately call `lighter.deposit.prepare` in the same turn with `fundingAssessment.depositAmountIn`. Do not ask 'shall I prepare', do not ask for another confirmation, and do not stop at a prose plan. The deposit approval card is the consent surface. Never call `lighter.deposit` directly; only the trusted approval resume may execute it.");
  lines.push("- When live onboarding status returns `fundingAssessment.decision = insufficient_wallet_settlement_asset`, do not prepare a deposit. Show requested collateral, current Lighter collateral, the selected Vex wallet's environment-specific settlement asset (USDC on Core, USDG on RHC), required deposit, and exact wallet shortfall. Do not count ETH or other assets as depositable settlement collateral; any swap or bridge requires its own live quote and separate approval.");
  lines.push("- When live onboarding status returns `fundingAssessment.decision = below_lighter_deposit_minimum`, do not prepare a deposit and never round the top-up upward. Show requested collateral, current Lighter collateral, the selected Vex wallet's environment-specific settlement asset, the combined settlement balance, the exact required top-up, and Lighter's live minimum deposit. Explain that the user must choose a trade or funding amount whose required deposit meets the minimum.");
  lines.push("- When live onboarding status returns `tradeMinimumAssessment.decision = below_lighter_trade_minimum`, stop before any deposit preparation or approval card. Show the requested trade, the live market minimum, current Lighter collateral, the selected Vex wallet's environment-specific settlement asset, and their combined balance. Never increase the trade or deposit extra funds automatically; ask for a new amount at or above the venue minimum.");
  lines.push("- Preparing the exact shortfall deposit activates the local integration automatically; never send the user to Settings to enable Lighter.");
  ...
  lines.push("- When onboarding status returns `tradingAccessRoute.kind = prepare_key_registration_approval`, immediately call `lighter.key.register.prepare` in the same turn with the returned environment. Do not ask 'shall I prepare', do not stop at a prose plan, and never call `lighter.key.register` directly. The separate key-registration approval card is the user's consent surface.");
  ...
  lines.push("- If onboarding is already ready, proceed to the requested IOC market-order preview and ask only for ordinary trade inputs that are genuinely missing: market, side, size, and worst acceptable price. Phase 1 does not expose a choice of order type.");
  lines.push("");
  ...
  lines.push("- Phase 1 permits IOC market orders only. Refuse resting limit, good-till-time, and post-only requests and explain that Vex will enable them only with production-safe cancel and modify support. Treat `price` as the market order's worst acceptable execution price.");
  lines.push("- Build the preview in the same environment selected during onboarding. Generic Lighter requests use RHC; an explicit Core request stays Core. Never omit environment after onboarding has selected it.");
  ...
  lines.push("- Do not ask the user to confirm that you should place, execute, submit, or broadcast directly from a preview. Do not say a preview can be broadcast after only supplying an API-key index. Approval preparation is not live submission; the exact host approval must resume the privileged execution path.");
