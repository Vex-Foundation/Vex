...
/** How many example tool names each namespace entry shows (scent, not a menu). */
const EXAMPLE_TOOL_NAMES_PER_NAMESPACE = 3;
...
interface NamespaceSummary {
  ...
  hasMutating: boolean;
  /**
   * MODEL-VISIBLE `publicName`s, never `toolId`s. The dotted toolId is the
   * internal and audit identity only (`protocols/types.ts`); the catalog rejects
   * it as a call, so rendering one here would teach an uncallable name.
   */
  examplePublicNames: string[];
}
...
function buildNamespaceSummaries(): Map<ProtocolNamespace, NamespaceSummary> {
  ...
  for (const [ns, tools] of byNs) {
    ...
    summaries.set(ns, {
      ...
      examplePublicNames: available
        .slice(0, EXAMPLE_TOOL_NAMES_PER_NAMESPACE)
        .map(t => t.publicName),
    });
  }
...
export function buildProtocolsPrompt(): string {
  ...
  for (const group of getGroupedAdvertisedProtocolNavigation()) {
    ...
    for (const metadata of group.namespaces) {
      ...
      if (summary.availableCount === 0) {
        ...
        lines.push(`Examples: ${summary.examplePublicNames.join(", ")}`);
        ...
        // one-liner and three tool names and could not tell that e.g. launching a
  ...
  lines.push("- KyberSwap is the PRIMARY swap route and Khalani the PRIMARY bridge route. `SwapQuoteUniswap` / `SwapExecuteUniswap` and `BridgeQuoteRelay` / `BridgeExecuteRelay` are always callable alternatives, not the default choice: quote the primary venue first.");
  lines.push("- Switch venue when the primary CANNOT serve the trade: no aggregator support for the chain, a route or token it cannot price, a build or pre-sign check that its own route fails, the execute transaction reverting on-chain, or the venue being unavailable to us at all (refused at its edge, unreachable, rate limited, or erroring). The failure output names the alternative when switching is the right move.");
  lines.push("- Do NOT switch venue for a trade-condition failure. A bad price quote is not a reason by itself, and neither is a slippage, balance, allowance, or deadline failure: each of those clears with a fresh quote or a corrected amount, and the other venue will refuse them the same way. When you do switch, take a FRESH quote on the new venue before executing — an execute is authorized only by a matching quote from the SAME venue — and never resubmit the identical failing route.");
  ...
  lines.push("- Quote and execute on the SAME venue: a swap execute runs only against a fresh quote from the exact venue it will broadcast on (the same rule holds for every venue, not just `kyberswap`). The runtime enforces this.");
  lines.push("");
  ...
  lines.push("- PLANNING starts at `trench__images_list`: a launch REQUIRES an image the user pre-staged in the app and you can never supply one. If the locker is empty, do not improvise around it — ask the user to upload an image to the Trench Photos card, then continue.");
  lines.push("- `trench__launch_preview` is this path's preview under the `# Safety Contract`'s fresh-quote rule: it dry-runs the launch and reports the predicted token address, the creation fee, the 25 bps Vex fee (a separate transfer that runs only after the launch confirms), and the gas cost. Preview in the same turn you intend to execute.");
  ...
  lines.push("- A GRADUATED agent token trades against VIRTUAL on its chain's venue: `SwapQuote`/`SwapExecute` on EVM chains (Robinhood Chain, Base, Ethereum), `solana.*` on Solana. The `virtuals__agent_get` result's `tradingRoute` hint names the VIRTUAL quote-token address — use it.");
  lines.push("- ANTI-SNIPER: before buying a graduated agent, call `virtuals__agent_get` and check `antiSniper`. NEVER buy while `windowActive` is true — the buy tax starts near 99% at graduation and decays to ~1% over the window. Wait out `remainingSeconds`, or tell the user the token is inside its sniper-protection window.");
  ...
  lines.push("- Buying a PT locks funds until maturity. Exiting EARLY (`pendle__pt_sell`) is market-priced and CAN lose money versus the locked rate — say so before recommending a buy.");
  lines.push("- A MATURED PT redeems ~1:1 to its accounting asset via `pendle__pt_redeem`; value a matured PT at face, never at the underlying spot price.");
  lines.push("- A yield token (YT) is the OPPOSITE leg: `pendle__yt_buy` is VARIABLE, leveraged yield exposure that DECAYS TO ZERO at expiry and is worth nothing after it — NOT fixed yield, and it can lose money. Frame YT as a variable-yield bet, never as a guaranteed or fixed return; `pendle__yt_sell` exits early at the market price.");
  lines.push("- `pendle__rewards_claim` sweeps ACCRUED interest and rewards (from held YTs and LP positions) to the wallet WITHOUT closing any position — it moves only income, never principal.");
  lines.push("- `pendle__py_mint` splits ONE token into BOTH an equal PT and YT in a single transaction; `pendle__py_redeem` burns an EQUAL PT+YT pair back to a token BEFORE expiry. Both need a fresh matching `pendle__py_quote`; a MATURED PT (PT only, no YT) uses `pendle__pt_redeem` instead.");
  lines.push("- `pendle__lp_add` provides single-token liquidity (one token → the market's LP), which earns swap fees and rewards; `pendle__lp_remove` burns the LP back to one token. LP is NOT a fixed-rate lock: after expiry it stops earning and only the principal side remains removable. Both need a fresh matching `pendle__lp_quote`; approval-gated.");
  lines.push("- SY is Pendle's standardised-yield form of a yield-bearing asset — the token PT and YT are actually minted from. `pendle__sy_mint` wraps a plain ERC-20 into SY; `pendle__sy_redeem` unwraps SY back to a plain ERC-20. Neither buys a PT (`pendle__pt_buy`) nor mints a PT+YT pair (`pendle__py_mint`) — do not substitute one for the other.");
  lines.push("- `pendle__sy_redeem` is the FALLBACK-SY UNWRAP: when `pendle__pt_redeem` cannot reach Pendle's pricing service it falls back to a direct Router redeem that pays SY, NOT the market's underlying, and reports it as `deliveredAsset`. That exit is NOT finished — pass that `deliveredAsset` to `pendle__sy_redeem` as `sy` to convert it, and tell the user that is what happened rather than reporting the redeem as complete.");
  lines.push("- MOVING A TERM IS ITS OWN ACTION, not a sell followed by a buy. When a user wants to EXTEND a fixed rate, roll a maturing position, chase a better rate, or change maturity, reach for `pendle__pt_rollover` (PT → a later-expiry PT of another market) — it never puts the underlying in the wallet in between, and it reports the implied APY BEFORE and AFTER so you can say whether the roll actually improves the rate. Do not manufacture the same outcome from `pendle__pt_sell` + `pendle__pt_buy`. `pendle__lp_transfer` is the same primitive for liquidity (LP → another market's LP). For all of these the SOURCE may be MATURED — leaving an expired position is exactly the point — but the DESTINATION must be ACTIVE, and a matured destination is refused by name.");
  lines.push("- `pendle__lp_to_pt` converts an LP into the SAME market's PT in one step — variable pool exposure traded for that market's fixed rate. There is no underlying to choose: the PT you receive is that market's own, the optional `pt` param is only a CHECK, and a PT from a different underlying is refused rather than silently substituted. It needs an ACTIVE market; a MATURED LP should be withdrawn with `pendle__lp_remove` instead. To reach a DIFFERENT market's PT, use `pendle__lp_transfer` then `pendle__lp_to_pt`, or exit and buy.");
  lines.push("- `pendle__lp_remove_dual` and `pendle__lp_add_keep_yt` each produce TWO instruments where the plain LP tools produce one: removeDual burns the LP into a plain token AND the market's PT, addKeepYt deposits and hands you the LP AND the YT that `pendle__lp_add` would have sold into the pool. Report BOTH legs to the user — describing either as a single-asset result is wrong. Be honest about what does NOT exist: there is NO two-token deposit on Pendle, so addKeepYt is still a SINGLE-token add, and `pendle__lp_transfer` has NO keep-YT variant. Never offer one.");
  ...
  lines.push("- Before quoting an unfamiliar market, read it with `pendle__market_get` (by market, PT or YT address): it returns the legs, expiry, the tokens the market ACCEPTS, and Pendle's own current rates — passing a token that is not on those lists is the most common Pendle rejection. It is also the only Pendle read that resolves a MATURED market, which returns no live rates.");
  lines.push("- `pendle__market_history_get` (implied APY / underlying APY / TVL over time) and `pendle__market_candles_get` (PT, YT or LP price candles) answer whether today's rate is high or low for that market — a rate is never high or low on its own. `pendle__asset_prices_get` prices Pendle assets the wallet does not hold; both are display marks, never an executable quote.");
  lines.push("- `pendle__market_orderbook_get` shows resting limit orders. Vex quotes and trades through Pendle's AMM ONLY, so that depth is the price quality being forgone and Vex CANNOT fill it — never promise a user a price from it.");
  lines.push("- `pendle__merkle_rewards_list` reads campaign/incentive rewards accrued to the wallet. Vex can NEVER claim them: Pendle publishes the amount but not the proof a claim needs. Say so and point the user to app.pendle.finance; `pendle__rewards_claim` is for the accrued interest and rewards Vex can actually sweep.");
  ...
  lines.push("- Check liquidity before sizing — thin markets mean high price impact on exit. Always preview with `pendle__pt_quote` (or `pendle__yt_quote` for YT) first; PT/YT buy/sell/redeem require a fresh matching quote and are approval-gated.");
  lines.push("");
  ...
  lines.push("- IDENTIFIERS: a Morpho `marketId` is a 64-hex hash and is CHAIN-SCOPED, not a contract address. `morpho__market_get` needs both `marketId` and `chain`; the same id on the wrong chain resolves to nothing.");
  ...
  lines.push("");
  lines.push("- Between two Khalani-supported chains, bridge with `BridgeQuote` then `BridgeExecute` (they auto-route to `khalani.*`). The live chain list is in the turn state.");
  lines.push("- Quote and execute on the SAME bridge provider (`khalani` or `relay`). The runtime enforces this.");
  lines.push("- Reads on Robinhood Chain go direct-RPC: `WalletBalances` for balances, `ChainRead` for tx receipts / ERC-721 mints / a direct `erc20_balance` read (alias `robinhood` / id 4663). `khalani__token_balances_get` does NOT cover it.");
  lines.push("");
...
export function buildBridgeCapabilityPrompt(view: BridgeCapabilityView): string {
  ...
  if (view.kind === "available" && view.robinhoodViaRelay) {
    ...
    lines.push(
      ...
      // the shortcut table in `# Tool Model` (`BridgeQuote`/`BridgeExecute` auto-route
