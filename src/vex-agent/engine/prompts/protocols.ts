...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("");
  lines.push("- Treat plain-language requests such as \"set up my Lighter account\", \"get me ready to trade on Lighter\", or \"I want to trade perps on Lighter\" as managed onboarding requests. First inspect the selected Vex EVM wallet's onboarding status in the requested Lighter environment (Core by default, Robinhood Chain when requested). Omit wallet addresses, account indexes, API-key indexes, nonces, fingerprints, and key fields from normal-user questions; Vex resolves them internally.");
  ...
  lines.push("- For a trade request stated as a USDC amount or 'USDC worth', pass that amount as `amountIn` and the named asset as `marketSymbol` to onboarding status. This checks the live market minimum before funding and is not permission to deposit or trade. Never omit the market for a named trade request.");
  ...
  lines.push("- When live onboarding status returns `fundingAssessment.decision = below_lighter_deposit_minimum`, do not prepare a deposit and never round the top-up upward. Show requested collateral, current Lighter collateral, selected Vex wallet USDC, combined USDC, the exact required top-up, and Lighter's live minimum deposit. Explain that the user must choose a trade or funding amount whose required deposit meets the minimum.");
  lines.push("- When live onboarding status returns `tradeMinimumAssessment.decision = below_lighter_trade_minimum`, stop before any deposit preparation or approval card. Show the requested trade, the live market minimum, current Lighter collateral, selected Vex wallet USDC, and their combined balance. Never increase the trade or deposit extra funds automatically; ask for a new amount at or above the venue minimum.");
