...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- SIXTEEN MORPHO TOOLS: ten reads and previews, plus SIX that move real funds. On VAULTS: `morpho.vault.deposit` and `morpho.vault.withdraw`. On BLUE MARKETS: `morpho.market.supplyCollateral`, `morpho.market.borrow`, `morpho.market.repay` and `morpho.market.withdrawCollateral`. Vex can now run the whole borrowing lifecycle on a market it vouches for, so it CAN act on a position near liquidation: supplying collateral or repaying debt both raise the health factor. Every one of the six takes `dryRun` for a rehearsal that signs nothing and records its outcome in the activity ledger.");
