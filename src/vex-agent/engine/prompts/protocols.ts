...
export function buildProtocolsPrompt(): string {
  ...
  lines.push("- When live onboarding status returns `fundingAssessment.decision = insufficient_wallet_usdc`, do not prepare a deposit. Show requested collateral, current Lighter collateral, selected Vex wallet USDC, required deposit, and wallet USDC shortfall. Do not count ETH or other assets as depositable USDC; acquiring USDC requires its own live quote and separate approval.");
