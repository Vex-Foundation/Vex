-- 099 - Z500 allocation-sync run ledger (indexiy-ansem.md).
--
-- RUNS AFTER 098. (Originally authored as 095; renumbered when upstream's
-- studio branch claimed 095-098 first — this file had only ever run in
-- ephemeral test containers, so the renumber-is-forbidden rule for shipped
-- migrations does not apply.) One table, one job: make the daily Indexify allocation
