-- separate approval-gated Ethereum claims.
--
...
CREATE TABLE IF NOT EXISTS lighter_withdrawal_claim_attempts (
  ...
  -- POSSIBLE send attempt, never proof that bytes reached Lighter: once it
  -- is set the row may only move on provider evidence or reconciliation,
  -- never by expiry or cancellation.
