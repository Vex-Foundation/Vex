...
-- successively restated one CHECK, this repair installs the final 084 shape
...
-- 079, 082, 084: final AgentScan predicates (lend + pools + transfer union).
...
ALTER TABLE agent_activity
  ...
  CHECK (event_role IN (
    ...
    'wrap','unwrap','yield_pt','yield_yt','yield_py','yield_lp','yield_sy','yield_claim',
    'token_launch','trench_fee','swap_fee','pools_fee','pools_claim','wallet_transfer'
  ));
...
ALTER TABLE agent_activity
  ...
  CHECK (
    ...
    OR (kind = 'bridge' AND event_role IN ('allowance_reset','allowance','bridge_deposit','bridge_fee','bridge_fill_expected','bridge_fill_observed','bridge_refund'))
    OR (kind = 'lend' AND event_role IN ('allowance_reset','allowance','lend_deposit','lend_withdraw','lend_borrow_operate'))
    OR (kind = 'prediction' AND event_role IN ('predict_buy','predict_sell','predict_claim','predict_close'))
    OR (kind = 'wrap' AND event_role IN ('wrap','unwrap'))
    OR (kind = 'yield' AND event_role IN ('allowance_reset','allowance','yield_pt','yield_yt','yield_py','yield_lp','yield_sy','yield_claim'))
    OR (kind = 'launch' AND event_role IN ('allowance_reset','allowance','token_launch','trench_fee','pools_fee'))
    OR (kind = 'claim' AND event_role IN ('pools_claim'))
    OR (kind = 'transfer' AND event_role IN ('wallet_transfer'))
  );
