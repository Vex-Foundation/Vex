-- 110 - Z500 allocation-sync run ledger (indexiy-ansem.md).
--
-- RUNS AFTER 106, PARKED AT 110: 107-109 are deliberately left free for
-- upstream's own fix of its duplicate-102 collision (see NOTE below), so a
-- third renumber of this file cannot be forced. (Originally authored as
-- 095, then 099; renumbered each
-- time upstream claimed the number first — this file has only ever run in
-- ephemeral test containers, so the renumber-is-forbidden rule for shipped
-- migrations does not apply. NOTE: upstream main itself ships duplicate 102
-- prefixes — launchpad_family_roles and portfolio_snapshot_group_wallets —
-- which is upstream's own collision to resolve, not this branch's.) One table, one job: make the daily Indexify allocation
