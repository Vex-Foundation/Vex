-- Durable approval-gated Core USDC withdrawals.
--
...
CREATE TABLE IF NOT EXISTS lighter_withdrawal_intents (
  ...
  execution_state              TEXT NOT NULL DEFAULT 'approval_pending'
    CHECK (execution_state IN (
      ...
      -- aborted, with no submission attempt started. Terminal: the signing
      -- evidence is retained, the withdrawal was never sent. The truth clause
      -- lives in 136, with send_attempt_started_at.
      'expired_unsubmitted'
    )),
