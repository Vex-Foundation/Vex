-- Lighter order submit lifecycle fields.
--
...
ALTER TABLE lighter_order_execution_intents
  ...
  -- Set in the same statement that immediately precedes sendTx. It marks a
  -- POSSIBLE send attempt, never proof that bytes reached Lighter: once it is
  -- set the row may only move on provider evidence or reconciliation, never by
...
ALTER TABLE lighter_order_execution_intents
  ...
  CHECK (
    ...
    AND (
      ...
      OR execution_state NOT IN ('signed','submitted','api_accepted','ambiguous','expired_unsubmitted')
    )
  ) NOT VALID;
