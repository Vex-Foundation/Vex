/**
 * LEGACY Trench Express launch settlement decoding: `TokenCreated`, and `Bought`
 * when a prebuy rode along in the same transaction. Read-only history; see
 * `./constants.ts` for why this survives.
 *
...
import { LEGACY_TRENCH_EVENT_ABI } from "./events-abi.js";
...
function decodeBoughtAmount(
  ...
  for (const log of logs) {
    ...
    try {
      ...
      // POSITIONS PROVEN BY THE FUNDED PROBE (pinned in the legacy decoder test):
