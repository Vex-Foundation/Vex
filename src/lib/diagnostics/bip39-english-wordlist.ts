/**
 * The standard English BIP39 mnemonic wordlist (2048 words), vendored as a
 * dependency-free data module.
 *
 * Provenance: extracted from the repository's existing `ethers` dependency
 * (`wordlists.en`, which is the canonical BIP39 english list). It is vendored
 * rather than imported because this module must stay importable from
 * `vex-app` (main/preload/renderer) through the `@vex-lib` alias, and
 * `ethers` is not a `vex-app` dependency — importing it here would drag the
 * whole library into the Electron bundles.
 *
 * Regenerate (from the repository root) with:
 *   node -e 'const {wordlists}=require("ethers");const w=wordlists.en;
 *     console.log(Array.from({length:2048},(_,i)=>w.getWord(i)).join(" "))'
 *
 * Stored as one space-separated string so the source file stays small; the
 * lookup `Set` is built once at module load.
 */

const ENGLISH_WORDLIST_RAW =
  ...
  "design desk despair destroy detail detect develop device devote " +
  ...
  "drip drive drop drum dry duck dumb dune during dust dutch duty dwarf " +
  ...
  "great green grid grief grit grocery group grow grunt guard guess guide " +
  ...
  "loan lobster local lock logic lonely long loop lottery loud lounge " +
  "love loyal lucky luggage lumber lunar lunch luxury lyrics machine mad " +
  "magic magnet maid mail main major make mammal man manage mandate mango " +
  ...
  "reveal review reward rhythm rib ribbon rice rich ride ridge rifle " +
  ...
  "sail salad salmon salon salt salute same sample sand satisfy satoshi " +
  "sauce sausage save say scale scan scare scatter scene scheme school " +
  ...
  "whale what wheat wheel when where whip whisper wide width wife wild " +
  "will win window wine wing wink winner winter wire wisdom wise wish " +
...
/** Membership test for the standard English BIP39 wordlist. */
export const BIP39_ENGLISH_WORDS: ReadonlySet<string> = new Set(
  ENGLISH_WORDLIST_RAW.split(" "),
);
